...
3. Thread/Chat 在经授权的 Git Workspace 中持续工作，后续 Turn/Run 不重建 checkout；高风险行为经过可追踪审批。
...
3. 每个用户的身份、Profile Home、配置、Secret、Thread、记忆与扩展状态隔离；每个 Thread/Chat 使用经授权且跨 Turn/Run 保持稳定的 Workspace，Run 的事件、审批和交付权限独立审计，并有跨用户负向测试证明。
...
- 每个 Thread/Chat 关联一个经授权的可写 Workspace，并在其后续 Turn/Run 中完成 Diff、Commit 和 Push。
...
| Workspace | Thread/Chat 执行所使用的经授权 Git checkout；托管 Worktree 默认绑定 Thread | Git + Runner |
...
- 一个 Profile 可以处理多个已授权 Workspace，但 Profile Host 必须先验证 Workspace 与 Thread 的关联。
- 托管 Workspace 默认属于 Thread/Chat，并跨 Turn 和平台 Run 保持稳定；Run 只引用 Workspace，不创建、拥有或替换 checkout。
- Task 恢复可以创建后继 Run 并在原 Thread 的 Workspace 中继续；本地或永久 Workspace 只有经过显式选择和授权时才能被多个 Thread 使用。
...
authenticated user
  ...
  -> task / codex thread
  -> profile / workspace
  -> run / approval / event / artifact
```

- 数据库查询不能只凭资源 ID 命中后返回，必须同时验证组织、成员关系、状态和动作权限。
- Profile Host 必须验证 `profile_id + user_id + thread_id + workspace_id`，Runner 必须验证 Run 关联同一 Task/Thread/Workspace；浏览器不能提供可信本地路径。
...
- 正常：创建新 Thread 时，Runner 为其创建或显式关联经授权 Workspace；调度器领取 Run 后解析该 Thread 的 Workspace，Profile Host 启动/复用 app-server，完成合同握手并创建或恢复 Thread。后继 Run 不创建新的 checkout。
...
- 正常：浏览器重连补发事件；Server/Host 重启后从数据库、Profile 和 Git 三方核对状态；用户可创建后继 Run，在原 Workspace 中继续原 Thread。
...
- 正常：归档 Task，终止活动 Run，按保留期清理该 Thread 独占的托管 Worktree 和 Artifact，保留审计与 Thread 映射；仍被其他授权 Thread 使用的 Workspace 不得删除。
...
| RUN-003 | P0 | 每个 Run 必须解析并校验其 Thread 的 Workspace，不能创建、拥有或隐式切换 checkout |
...
| 托管 Worktree | Thread/Task 归档后 7 天 | 1–30 天 | 清理前确认无活动 Run、未保留变更且无其他授权 Thread 引用 |
...
- 每个 Thread/Chat 使用经授权且跨 Turn/Run 稳定的 Workspace；Run 不创建独立 checkout，普通用户不能注册任意服务器路径。
