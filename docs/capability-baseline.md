...
| Enterprise Supervisor Policy | built-in `1.7.0` exact role/capability isolation verified; user Supervisor and Agent Release publication DB-verified | The catalog accepts either a reviewed repository Package or an immutable organization-scoped user Release. Supervisor Studio can save a draft, validate exact Agent and Artifact handoffs, publish a version and expose it through the same generic Run selector. It can select a code Agent or an exact organization Agent Release; custom dependencies persist the Release UUID, version and content hash. Browser requests do not contain Runtime Role names, MCP inventory or Runtime capability claims; the server derives those facts from the resolved Agent catalog. Published Releases retain exact spec, content hash and release identity, and worker preflight re-resolves both Supervisor and Agent identities before Thread creation. A one-Agent Supervisor is accepted for Hello-Agent-style packages. The real built-in journey still proves that request-scoped root configuration selects only Definition-declared capability roots, disables ordinary Apps/Plugins/shell, supplies an exact Role allowlist and one resident instance per required Role, while child Role files disable V2 delegation. The root has no business MCP tools; Role files remain Profile Host materialized, hash verified and absent from persistent Profile/Project Agent configuration. Disposable PostgreSQL tests cover fresh migration, exact publish/resolve and cross-Organization denial. A real Runtime smoke of a user-authored Agent Release remains pending |
| Enterprise Agent Definitions | built-in Data `1.6.0`/Network `1.5.0` Runtime-verified; user publication DB-verified | Code-managed Definitions and reviewed instructions have one publication source under `capabilities/agents/`; the typed catalog exposes responsibilities, input/output Artifact types and required capabilities without exposing Runtime Role names. Agent Catalog now creates organization-scoped Definition/draft Revision resources, validates and publishes immutable Releases, and returns code and user Releases through one browser catalog. A custom Agent must bind one exact code-reviewed capability template and may only narrow its Artifact inputs/outputs; the server derives the Role name/path, MCP servers, Tool allowlist and capability roots. Generated Role TOML disables shell, ordinary Apps/Plugins, V2 and child delegation, then re-enables only the template-declared plugin MCP server with an exact `enabled_tools` allowlist. Supervisor Releases bind a custom Agent by exact Release UUID/version/content hash, and preflight fails closed on missing or drifted dependencies. Native Runtime Agent CRUD, arbitrary capability/Tool authoring, deprecation and a real Runtime execution of a custom Release remain unsupported |
...
| Persistence | PostgreSQL migrations cover users/sessions, organizations/memberships, Profiles/capabilities/encrypted Secrets, projects, tasks, Runs, leases, Workspaces, durable approvals/audit, Agent and Supervisor Definitions/draft Revisions/immutable Releases, exact Supervisor-to-Agent Release dependencies, Policy snapshots/bindings, rebuildable Runtime Agent tree and child-task execution observations (`runtime_agent_projections`, `runtime_agent_execution_projections`), Task-owned typed Artifacts with producer provenance, and versioned Run-event projections. Release metadata is governance state; generated Runtime Role configuration is not duplicated in PostgreSQL. A disposable fresh-schema test proves Agent/Supervisor publication resolution and cross-Organization denial; existing tests also prove deleting a producer Run does not delete its Artifact, Task grant or immutable provenance identity | Release deprecation/deletion policy, concurrent edit conflict UX, Artifact replacement/invalidation/deletion/retention/cross-Run reuse and complete multi-user constraints remain missing |
...
| Browser | established WebApp presentation runs through typed platform resources for workspace/thread/message, approvals, Provider/model, MCP/rate-limit snapshots, files, Git status and Agent/Supervisor governance. Agent Catalog creates, saves, validates and publishes organization Agent versions by choosing a reviewed capability template and Artifact subset. Supervisor Studio lists code and organization Agent responsibilities/contracts, selects exact Releases and publishes immutable policies. The generic Workspace action lists both built-in Packages and organization Releases and starts the exact selected version. The browser never submits Runtime Role names, MCP inventory, Tool allowlists, capability roots, local paths or release content at Run start. The existing Agent/Files sidebar continues to render root/child projections, immutable child task nodes, bounded activities, waiting states and Task Artifacts without exposing reasoning, tool arguments/results, internal Resource URIs or host paths | add native Agent/Skill/Plugin/MCP lifecycle, make child Agent task nodes navigable to authoritative histories, add Artifact content/open actions, then cover Release deprecation, concurrent editing, deeper trees, failure/approval states, narrow layouts, shared-Workspace concurrency and broader cross-user denial |
