...
| Enterprise Supervisor Policy | real single-Profile case validated | The code-published `enterprise-supervisor-copilot@1.0.0` Policy is resolved server-side, hashed into an immutable Snapshot, bound to the Run and actual root Thread, and injected only through official `thread/start.developerInstructions`. Resume and idempotent scheduling retain the original binding. Run start requires the generated Capability Manifest to declare `agents.multi_agent@1.0.0` as supported or explicitly experimental, then rejects disabled Profile configuration, insufficient Runtime limits or either missing exact Runtime Role; it never falls back to a default role. The real warehouse case completes under the bound version and the browser restores the same binding |
| Enterprise Agent Definitions | real exact-role spawn validated | Code-managed `enterprise-data-agent@1.0.0` and `enterprise-network-planning-agent@1.0.0` manifests declare responsibilities, Artifact inputs/outputs, required capabilities, risks and exact `data_agent` / `network_planning_agent` Runtime Role references. Platform startup gates the enterprise Run against official Profile configuration without installing or mutating hidden role files. The real journey proves both configured Roles are discovered and used by child `thread/started` provenance; unknown-role failure and broader catalog governance remain separate tests |
