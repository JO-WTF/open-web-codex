...
| Enterprise Supervisor Policy | `1.7.0` exact role/capability isolation verified | The generic Workspace action lists the one current Policy from the bounded server catalog and submits its ID/version; browser code contains no supply-chain Policy ID, version or Agent order. The server neither publishes nor resolves historical Policy or Agent Definition versions. The current supply-chain Policy requires exact `data_agent` then `network_planning_agent` stages. Request-scoped root configuration selects only the capability roots declared by the bound Agent Definitions, disables ordinary Apps/Plugins/shell, supplies an exact `agents.allowed_roles` set, and seals a maximum of one resident instance for each required Role. Runtime declares and enforces both exact-role-allowlist and exact-role-instance-limit capabilities, so built-in roles cannot bypass the Policy and duplicate role spawns cannot consume another stage's capacity. Child Role files explicitly disable V2 delegation. The root has no business MCP tools; Role files remain Profile Host materialized, hash verified and absent from persistent Profile/Project Agent configuration. A completed root Turn transactionally terminates the bound Run/Task. Prompt instructions remain defense in depth rather than the authorization boundary |
| Enterprise Agent Definitions | Data `1.6.0`, Network `1.5.0` verified | Code-managed `enterprise-data-agent@1.6.0` and `enterprise-network-planning-agent@1.5.0` manifests declare responsibilities, Artifact contracts, required capabilities, owning capability-root IDs, risks and exact Runtime Role references. Their generated Role TOML disables shell, ordinary Apps/Plugins, V2 and child delegation, then re-enables only the declared plugin MCP server with an exact `enabled_tools` allowlist. No historical Definition catalog is retained. Only bound Supervisor preflight materializes Role files, and request-scoped Runtime configuration exposes the selected Roles without polluting the Profile Agent catalog. Native Agent CRUD and broader Catalog governance remain unsupported |
...
| Persistence | PostgreSQL migrations cover users/sessions, organizations/memberships, Profiles/capabilities/encrypted Secrets, projects, tasks, Runs, leases, Workspaces, durable approvals/audit, immutable Supervisor Policy snapshots/bindings, rebuildable Runtime Agent tree and child-task execution observations (`runtime_agent_projections`, `runtime_agent_execution_projections`), Task-owned typed Artifacts with producer provenance, and versioned Run-event projections. Runtime Role configuration is deliberately not duplicated in PostgreSQL. The latest validated enterprise case materialized ten ready Artifacts and read them across child Threads without exposing internal Resource URIs or host paths. Fresh-schema tests prove deleting a producer Run does not delete the Artifact, its Task grant or immutable provenance identity | Artifact replacement, invalidation, deletion, retention, cross-Run reuse and complete multi-user constraints remain missing |
...
| Browser | established WebApp presentation runs through typed resources for workspace/thread/message, approvals, Provider/model, MCP/rate-limit snapshots, Supervisor Policy catalog, files and Git status. Workspace listing and file/Git/terminal/GitHub operations use Workspace IDs directly; Thread/Turn and MCP operations retain Run provenance where the Runtime Thread is required. Thread creation selects an existing Workspace and creates only Task/Run records. A generic Workspace action opens the server-published Supervisor Policy catalog and starts the exact selected version; the browser cannot submit Policy contents or encode domain Agent order. The latest real exact-prompt warehouse Thread persisted the bound `1.6.0` Policy, root plus two Runtime Agents, nine ready evidence Artifacts, the human-readable collaboration trace and the complete final report. The Agent/Files right sidebar keeps both tabs selectable; the header Agent shortcut becomes active after a real child Agent appears. Its pinned Supervisor summary shows the main task, current status, safe behavior and latest public progress. Child work reads persisted task nodes bound to real Runtime Turns, so parallel Threads render independently, a completed node remains immutable and reusing the same Agent Thread creates a new node. The sidebar also renders every bounded persisted activity in sequence with actor, action kind, status, timestamp and public detail. Targetless V2 waits show an explicit active bounded-wait explanation in both live and restored ordinary Thread history instead of an empty output. Background Agent updates produce an unread indicator without replacing the active Files tab. Runtime projections omit tool arguments/results, reasoning, internal Resource URIs and host paths while retaining useful public context | make child Agent task nodes navigable to their authoritative histories, add Artifact content/open actions, then cover deeper trees, failure/approval states, narrow layouts, existing-root registration, shared-Workspace concurrency and cross-user denial |
