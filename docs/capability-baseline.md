...
| 文档性质 | 当前事实与证据 |
| 观察日期 | 2026-08-23 |
| 代码快照 | 阶段三 Runtime owner 收敛 Atom 2b；阶段一仓网与阶段二开发者 SDK 验收作为已通过基线 |
...
| Browser 产品入口 | 生产 `index.html` 只经 `browser-entry.ts` 渲染 `WebApp`；旧 `main.tsx`/`App` 仍不在 production graph，但仍由 `tsconfig` 编译。阶段三 Atom 1 删除 generation/remembered-approval，Atom 2a 删除 Profile file/Agent/Prompt client、facade 和 UI 链，Atom 2b 删除 Browser Workspace Preferences 全链 | E2 单一生产入口；剩余 Terminal/Usage legacy surface 继续按 owner 拆成独立 Atom 收缩 |
...
| `B-BROWSER-LEGACY` | 阶段三进行中 | 生产只有 `browser-entry.ts`→`WebApp`；Atom 1 已删除 generation/generic RPC/remembered approval，Atom 2a 已删除 Profile content/Agents/Prompts，Atom 2b 已删除 Workspace Preferences；剩余 dead UI/live API 为 Terminal 和 Usage | 继续维护第二 UI/API 表面会让 Platform 与 Runtime/Runner owner 冲突 | 继续按 owner 把 Terminal、Usage 分成独立 Atom 删除，不建替代子系统或兼容 facade | Browser 只保留 WebApp typed surface；Server 路由按 production reachability 收敛 | Atom 2b 已完成；Terminal、Usage 继续最先处理 |
...
Preferences、假 Runtime 参数和 setup/launch 脚本链，没有增加替代缓存、兼容 facade 或新的持久状态。
