...
| 文档性质 | 当前事实与证据 |
| 观察日期 | 2026-08-23 |
| 代码快照 | 阶段三 Runtime owner 收敛 Atom 3；阶段一仓网与阶段二开发者 SDK 验收作为已通过基线 |
...
| Browser 产品入口 | 生产 `index.html` 只经 `browser-entry.ts` 渲染 `WebApp`；旧 `main.tsx`/`App` 仍不在 production graph，但仍由 `tsconfig` 编译。阶段三 Atom 1 删除 generation/remembered-approval，Atom 2a 删除 Profile file/Agent/Prompt client、facade 和 UI 链，Atom 2b 删除 Browser Workspace Preferences 全链，Atom 3 删除独立 Terminal 全链 | E2 单一生产入口；剩余 Usage legacy surface 继续按 owner 作为独立 Atom 收缩 |
...
| `B-BROWSER-LEGACY` | 阶段三进行中 | 生产只有 `browser-entry.ts`→`WebApp`；Atom 1 已删除 generation/generic RPC/remembered approval，Atom 2a 已删除 Profile content/Agents/Prompts，Atom 2b 已删除 Workspace Preferences，Atom 3 已删除独立 Terminal | 继续维护第二 UI/API 表面会让 Platform 与 Runtime/Runner owner 冲突 | 继续按 owner 删除 Usage，不建替代子系统或兼容 facade | Browser 只保留 WebApp typed surface；Server 路由按 production reachability 收敛 | Atom 3 已完成；Usage 继续最先处理 |
...
10. `/profile/usage` 混用本地 `run_events` 近似值与 official account usage，并把
...
Preferences、假 Runtime 参数和 setup/launch 脚本链，Atom 3 已删除独立 Terminal，没有增加替代缓存、
兼容 facade 或新的持久状态。
旧跨 owner 数据面与 compatibility island 已删，
后续顺序固定为 Browser legacy 中剩余的 Usage → Provider/model owner → Run/Thread 与 history/event；
