...
| Multi-agent trajectory | declared experimental | The Server persists rebuildable root/child Thread and child-task execution projections from official collaboration, Thread, Turn, Item and approval events, associates child events with the authorized root Run, and prevents child lifecycle events from completing or otherwise mutating the root Run. Read-only `/api/runs/{run_id}/agents` and `/api/runs/{run_id}/agent-executions` resources expose bounded browser facts. Each observed child Turn has a stable task node; terminal rows are frozen, a later Turn on the same Agent Thread receives the next ordinal, and different child Threads can remain active concurrently. PostgreSQL integration covers prompt/event reordering, concurrent child activity, same-Agent reuse, terminal immutability and cross-organization denial. The previously validated real enterprise journey proves ordered spawn/wait execution for one root and two exact-role child Threads, child MCP activity association, browser refresh and Server/Profile Host restart recovery. The journey now also contains task-execution and recovery assertions, but the current post-change run stopped before Agent execution because the required Supply-chain MCP startup projection was absent; this new read model is therefore PostgreSQL- and Web-validated, not yet revalidated by a complete real journey. General interrupt, deeper trees, partial failure and multi-user isolation remain required before product support can be promoted beyond the constrained case |
| Enterprise Supervisor Policy | real single-Profile case validated | The code-published `enterprise-supervisor-copilot@1.0.0` Policy is resolved server-side, hashed into an immutable Snapshot, bound to the Run and actual root Thread, and injected only through official `thread/start.developerInstructions`. The Snapshot digest seals the Supervisor instructions and every referenced Agent Definition id/version/Runtime-instruction digest. Immediately before a governed root start or inherited fork, the worker re-resolves the published Policy, rejects Snapshot drift, requires `agents.multi_agent@1.0.0`, and asks Profile Host to atomically materialize the immutable Role files. Adapter reopens the files without following links and checks SHA-256 immediately before Runtime consumption. The official per-thread config enables only `features.multi_agent_v2`, sets the V2 concurrency limit and carries the exact Roles. It does not register enterprise Roles or backend choices in Profile/Project config and does not add a V1 compatibility selector or private V2 capability. The previous `profile_runtime_role_projections` second state machine has been removed. The post-refactor real warehouse E2E passes 9/9 and verifies V2 metadata for all three governed Threads |
| Enterprise Agent Definitions | real request-scoped exact-role spawn validated | Code-managed `enterprise-data-agent@1.0.0` and `enterprise-network-planning-agent@1.0.0` manifests declare responsibilities, Artifact inputs/outputs, required capabilities, risks, exact `data_agent` / `network_planning_agent` Runtime Role references, and immutable instruction digests. Only the bound Supervisor preflight materializes their internal Role files; request-scoped Runtime config makes them visible to that governed Thread without polluting the Profile Agent catalog. The post-refactor real journey proves both exact Roles spawn child Threads. Native Agent CRUD and broader Catalog governance remain unsupported |
