...
| 仓网算法 | Data4 标准化保留完整的用户路线 pair facts；Network13 可物化 provided/haversine/navigation 路线矩阵，计算成本、场景、求解、报告、地图，并返回城市数量与需求量加权两种覆盖指标 | E2；完整 Python 定向测试和真实 stdio inventory/domain E2E 通过，Web 产品链证据另行判断 |
...
| `P1-SC-DATA4` | 当前关键链 | `data_server.py` active path 已删除 hash `source_ref` 重扫和多套 alias/wire shape；inspect 产生 source-profile Resource，normalize 接显式 confirmed decisions 并可返回 `needs_geography`，prepare-geography 接受 adapter 已验证的行政区 catalog 与 overrides | 默认印尼数据准备链曾不可达，模型承担协议翻译或猜字段 | Data4 strict surface 已落地：discover 可选、一次 inspect、typed `ResourceRef`、显式 mapping、分阶段 geography；Indonesia built-in asset 只属于验收 fixture，其他国家无 provider 时 typed unavailable | Tool 拥有格式/映射/标准化；复用 `workspace_intake.py`、`mapping.py`、`normalization.py` 中纯函数与 `geography.py`，不建 source/revision/CAS 包装 | Data4 与 Network13 active surface 已落地；Stage E non-active tail remains |
| `P1-SC-NET9` | 当前关键链 | `server.py` 的 13 个 decorated active Resource tools 已使用 strict `ResourceRef`；Data Resource 保留完整 provided route facts，Network 可按 scope 物化路线矩阵，并确定性返回双覆盖口径；`ArtifactRef | DataAgentRef`、`_case_from_input_ref`、`CaseRepository` 及 mapping/matrix/analysis/scenario service 仍在 non-active compatibility island | Data4 输出的 active path 已可用单一 ref，非 active Case tail 仍形成第二业务状态和维护面 | 保持 current decorated strict `ResourceRef` surface；路线/cost 由 pair/lane fact 自主部分复用，Stage E 原子删除 Case/ArtifactRef compatibility tail | Network Tool 拥有算法与 pair facts；保留 `matrix.py`/`solver.py` 纯算法，删 `case_repository.py` 及 Case service/schema/tests | Active surface landed，provided facts/双覆盖口径的 Python 与真实 stdio 门通过；Stage E tail remains |
| `P1-ART-FINAL` | 当前关键链 | generic ResourceLink→Artifact、Run-scoped通用 inline 表和 Artifact 输入回流已删除；durable map/report 只认 built-in exact final Tool + typed Workspace-relative bundle。用户当前分布展示另有 exact `map_utils/create_map_card` bounded 投影，Resource 字节仍由 provider 持有 | 把展示卡片误写成文件会触发不必要审批并制造交付物；把任意 Resource 升为 Artifact 会形成第二数据面 | 展示请求只走 `normalized Resource→distribution GeoJSON Resource→create_map_card→embed`；只有明确导出/下载才走 final Tool/Artifact | Platform 拥有卡片授权投影和显式最终交付物化；MCP provider 拥有 Resource 内容；完整 Browser 自然语言门通过后封口 | 实现与定向门在途；完整 Browser 三案例未验收 |
