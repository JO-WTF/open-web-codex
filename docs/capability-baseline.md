...
| 文档性质 | 当前事实与证据 |
| 观察日期 | 2026-08-27 |
| 代码快照 | Codex recorded integrated base 为 `76d98a771e6c`；七个 retained Runtime seam 已完成 deterministic 验证及当前 HEAD 的 DeepSeek Flash Web 产品验收 |
...
2026-08-24 在同步 Codex 至 `76d98a771e6c`、比较地图升级为 v3、Data inspection 收敛为 v3 且真实
E2E 信封检查完成后，又用隔离 DB/Profile 和当前 HEAD 完成同样三轮 DeepSeek Flash Web 复验；
该证据不扩大成多用户、完整失败矩阵或 Marketplace 能力。
...
### 2026-08-24 阶段三当前 HEAD 真实 Web 复验

Root 在隔离 DB/Profile 的 Web 中新建 `Phase3 Final Acceptance v3` Workspace，上传 Downloads/mock_data
的 6 个文件，选择多 Agent 仓网 Copilot 和 `deepseek-v4-flash`。三轮均在同一 Task、同一 canonical
Root Thread 中完成，三张地图实际显示为 Ready：

| 业务问题 | Root Turn 用时 | 业务结果 |
| --- | ---: | --- |
| 12 小时时效达标率 | 132.533 秒 | 城市 33/50（66.0%）；需求量 41,399/50,815（81.5%）；17 城未达标 |
| 增加 Balikpapan 后的影响 | 212.594 秒，含一次用户选择 | 城市 37/50（74.0%，+8.0pp）；需求量 43,964/50,815（86.5%，+2,565、约 +5.05pp）；13 城未达标 |
| 90% 前提下最少仓库覆盖 | 139.146 秒 | 最少新增 Balikpapan、Jambi 两仓已证明；总启用 13 仓；需求量 45,771/50,815（90.0738%）；城市 38/50（76.0%） |

拓扑始终只有 Root、Data、Network 三个 Agent，两个 child Thread ID 在后续任务中保持不变。三轮
Tool Search 分别为 7、0、2 次：第二轮直接复用历史已加载 Tool，第三轮只搜索此前未加载的选址和
成本能力。正式复验没有 terminal failed Run，也没有 unknown Tool、`provider_tool_call_not_visible`、
stream disconnected 或 Provider 协议错误；4 次 typed Tool 参数/前置条件拒绝均由同一 child 修正后
继续完成，第二轮使用了一次 Runtime 原生用户输入卡片。

### 2026-08-27 Jakarta 真实导航与完整重启复验

Root 在同一个 `Phase3 Final Acceptance v3` Workspace/Task 中，用 DeepSeek Flash 对
`WH-CENTER-JAKARTA` 到全部 50 个需求城市执行真实驾车导航。最终全量重算从 Root Run 开始到回答
完成约 89 秒，其中 provider 导航执行为 56.805 秒；新结果文件含 50 个唯一目的城市、50 条
`ready`、0 条不可达和 0 条 provider error，全部为 Mapbox driving。Jakarta→Jakarta 的 provider
返回值为 0 公里、0 小时并按 `ready` 落盘，结果导入
`route_matrix.v3-4014f47ce94576ffd1350825`。随后又单独复验 1×1 路线：49 条 exact pair 复用、
1 条经 route endpoint 执行，落盘值仍为 0/0 `ready`，完整矩阵保持 50/50。

该复验还在完整 Profile/app-server 重启后先读取历史，再恢复同一个 Network child。首次 bind 从
Task 的 `copilot_package_id` 解析 trusted execution config，Runtime 恢复原 Role 的 supply-chain/Maps
MCP inventory；child 直接生成精确单路线请求，没有新建替代 Agent，也没有依赖 Platform MCP 缓存。
Maps stdio 通过声明 sandbox-state metadata capability 取得授权 Workspace，Maps 审批安全投影保留
`credentialKind=maps`，已加密保存的凭据由现有 typed credential 流自动交付。

测试序列有 3 次业务答复未达到 50/50：初次为 49/50，第一次补跑报告能力不可用，第二次补跑被
1×1 matrix 拒绝。诊断并逐 owner 修复了 6 个独立机制缺口：指定 Jakarta 被扩成 556 对、Maps 未
声明 sandbox metadata、审批投影把 Maps 凭据类型替换成通用脱敏值、零时长被当成缺失、冷重启的
首次 read/list/send 未携带 Task package execution、以及供应商 matrix 不接受单路线。最终门没有
unknown Tool、`provider_tool_call_not_visible`、stream disconnected、越权或结果伪造。

当前确定性回归为 Planner 193、Maps 43、Adapter 18、审批投影 41 项通过，`check:no-desktop`
通过；需要真实外部 Runtime/FastMCP 的 3 项 Adapter 测试按既有条件保持 ignored。
...
| Browser 产品入口 | 生产 `index.html` 只经 `browser-entry.ts` 渲染 `WebApp`；旧 `main.tsx`/`App` 仍不在 production graph，但仍由 `tsconfig` 编译。阶段三 Atom 1 删除 generation/remembered-approval，Atom 2a 删除 Profile file/Agent/Prompt client、facade 和 UI 链，Atom 2b 删除 Browser Workspace Preferences 全链，Atom 3 删除独立 Terminal 全链，Atom 4 删除本地 Usage 全链 | E2 单一生产入口；Browser legacy API 已清完 |
| Web UI parity | 历史 `check-main-ui-parity`、Git/UI overlay 和手工 SHA 清单已删除 | 已删除第二 UI truth；当前以类型检查、组件/合同测试、生产构建和真实浏览器验收为准 |
| Browser Terminal | 旧 `/workspaces/{id}/terminals*` 路由、独立 `command/exec` shell、`terminal_sessions`、输出/退出投影、Terminal UI/xterm 与 Workspace 删除门均已删除；已有 session 行由 migration 69 删除 | 已删除；Runtime Thread 内的 commandExecution、输出与 terminal interaction/approval 不变，不建设替代 selector |
| Browser local usage | `/profile/usage`、run_events token/Turn 聚合、`account/usage` fallback、LocalUsage DTO/client、Home/tray 使用量 UI 和相关设置均已删除；没有数据库迁移 | 已删除第二 usage owner；Runtime Thread token、`/profile/rate-limits` 与 `provider_call_metrics` 独立保留 |
...
| Browser Workspace preferences | `/browser-workspace-preferences*`、任意 JSON settings、未实际应用的 Runtime 参数、worktree setup/launch script 的 route、DTO、client/facade、旧 UI/hooks/tests 和 `browser_workspace_preferences` 当前表均已删除；没有读取或删除用户 Profile 文件。Sidebar 工作区卡片折叠只保留页面内状态；Workspace owner、GitRuntime 和 `/workspaces/{id}/git-roots` typed 能力保留 | 已删除；不保留兼容 API、旧表读取或未来 Workspace 配置 fallback |
...
| MCP Resource | Codex 官方 Tool Item/ResourceLink/read_resource 与供应链 Workspace-scoped ResourceStore 形成 Network typed provider 数据链；Data inspection 只返回 inline `workspace_source_profile.v3`、source units 和 `workspace_source_inspection.v2` identity，不注册 Data Resource template。已确认完整角色以 canonical→source compact resolved mapping 返回；只有未决 unit 保留字段证据，完整 JSON 严格限于 8192 bytes，超限为 typed selection。Network 通过 package-owned facade 使用 Profile+Workspace `supply_chain` Resource 域；ResourceRef/codec/bounds/store/runtime/Workspace primitives 已由领域无关的 `open-web-codex-provider-sdk` 提供，仓网只保留领域模型与算法。Platform 不存内容、不建 Broker、handoff ledger、latest 或相似性匹配 | focused 回归覆盖 inspection identity、5/6-source JSON budget、Data Resource surface empty、Network Resource scope/ref 拒绝、prepared fresh reuse 与 pair-level partial reuse；2026-08-24 当前 HEAD 三轮真实 Web 复验完成，Root Turn 分别为 132.533、212.594、139.146 秒 |
...
| 仓网算法 | Data4 标准化保留完整的用户路线 pair facts；Network12 通过一个 `prepare_route_matrix` 入口物化 provided/haversine 路线矩阵或给出 navigation 请求估算，计算成本、场景、求解、交互地图卡片数据和 Markdown 简报，并返回城市数量与需求量加权两种时效达标指标；`prepare_network_coverage_map` 从 exact assignment 生成通用覆盖 LineString。单次设施变更评估从 exact before result 的活动仓集合起算，一次求解并直接比较，返回 scenario ref、绑定标准化输入与前后方案的单一 `plan_comparison_ref` 及有界指标；`prepare_network_comparison_map` 使用比较结果中确切的 SLA 目标发布 `network_comparison_geojson.v3`，需求点按 after 状态、前后末程线按各自状态区分达标、未达标和未分配 | 路线规划/构建/显式重复验证和独立场景入口已从模型可见面删除；当前 HEAD 的三轮 DeepSeek Flash Web 计算和地图均通过 |
| Provider 定义与选择 | Profile `config.toml` 和 Runtime 唯一持久化 Provider/模型及未来 Thread 默认选择；已物化 Thread 的实际 pair 由 official Thread settings 拥有。Runtime Provider catalog 必填返回所有 Provider/model 字段、`supportsFunctionTools`、`supportsSearchTool` 和 nullable `currentModelId`；任一缺字段由 Platform/Browser 拒绝，隐藏模型只影响显示。`profile_provider_definitions` 镜像、Task pair、`models.default_selection` 和启动回放均已删除；migration 70/71 对旧开发数据明确拒绝并要求重建。Platform 保存 Chat=true/Responses=false 的产品声明但 Browser 不再传该字段。新 Thread 和 normal Turn 从不带 Provider/model override；同 Provider 的已物化 Thread 仅经 resume→settings/update(model)→resume 确认更新，跨 Provider 返回 `requiresNewThread` 并由 Browser 同 Workspace/Project 新建 Task/Thread | Runtime/Profile config 拥有定义、目录、Profile default 与 Thread settings；Platform 只留密文 Secret、授权、operation lock、Browser typed projection 与新 Thread 编排 |
| Provider 模型刷新 | Codex 的 provider-scoped `modelProvider/models/list` 对 exact 目标 Provider 执行 fresh `/models`，不切换 current Provider；非空 typed success 只写回 Runtime 完整模型目录，随后直接返回 Runtime 的 `modelProvider/list` catalog。单模型 capability/context 修改也只由 Runtime 完整目录写入；不读写或合并数据库模型目录，成功后才安排安全边界 Runtime refresh，空/失败不改变目录 | focused 回归覆盖完整 Runtime 目录原子写、缺必填 catalog 字段拒绝、目标 refresh 不切换 current Provider；真实重启同时核验 config 双模型和 Data child ToolSearch |
...
| Provider metrics | 旧 token event 不包含 Runtime 实际 provider/model，Platform 不再从 Task pair 填写 `provider_call_metrics`；历史读取 route/schema 保留，等待 Runtime 产生每次调用的 typed identity | 真实 usage 与 per-call identity 都未验收 |
...
| `P1-SC-DATA4` | 当前关键链 | `data_server.py` active path 的 inspect 返回 bounded inline `workspace_source_profile.v3`、exact source units、`record_count`、`record_count_exact`、`inspection_identity` 与 `inspected_relative_paths`；完整且无歧义角色以 canonical→source resolved mapping、confidence 与 ambiguity state compact 返回，只有 partial/ambiguous/未知表头 unit 保留字段类型、一个代表值、候选 mapping 和缺失字段。完整 structured JSON 严格限于 8192 bytes，超限返回 `selection_required` 而不截断；角色评估不构成全局业务缺口，只有选中 unit 的语义、单位、值或冲突无法确定时才询问。`prepare_network_input` 使用 `source_selections` 和显式 mapping，重检变化返回 `source_changed` 且不写 output；ready prepared 绑定 selected raw/admin provenance、roles、role_counts 与 `selected_source_identity`，fresh candidate 只有显式选中才自动复用，内容不同则一次选择。| 预览样本曾被模型误认为完整正文，且生成输入曾落在 Workspace 根目录或源数据目录；缺字段曾使 Data Agent 在 mapping 与 schema 错误之间重复调用而不询问；旧 source profile Resource 使 Data/Network 产生不必要的 read 链 | strict surface：discover→inspect→prepare，fresh prepared 为 discover→inspect→handoff；`needs_input` 一次询问并终止，`source_changed` 最多一次重检；Data `list_resources`/templates 为空；目录截断时不得断言候选仓不存在；Indonesia built-in asset 只属于验收 fixture，其他国家无 provider 时 typed unavailable | Data Tool 拥有格式/映射/inspection identity/标准化、typed 数据缺口和生成目录；Skill 只把业务终态转为一次询问，不建重试状态机；复用 `workspace_intake.py`、`mapping.py`、`normalization.py` 中纯函数与 `geography.py`，不建 source/revision/CAS 包装 | focused Data tests 覆盖 5/6-source JSON budget、`warehouse_type` 缺失 no-write、非标准表头、multi-sheet/nested JSON、order-independent identity、fresh reuse、selected raw/admin change、Balikpapan 不在 inline inspection 证据中仍可由完整读取准备、Data Resource surface empty 与两套 stdio domain gates |
...
| `P1-SC-NET9` | 当前关键链 | `server.py` 的 active Resource/final tools 使用 strict `ResourceRef`；`prepare_route_matrix` 统一 provided/haversine 入口并返回 discriminated `ready/needs_input`，只有零缺失矩阵才发布 `route_matrix.v3`；导航继续走显式 request/import 合同。`selected_warehouses` 将用户指定范围固定为精确仓库 ID，任何 Tool 不得隐式扩仓；Maps 声明 sandbox-state metadata 后只从 Runtime 注入的授权 Workspace 工作。批量路线走 provider matrix，单路线走同 provider route endpoint；合法 0 距离/0 时长按 `ready` 保存。baseline、设施变化和选址 Tool 独立拒绝不完整路线或成本。`assess_facility_change` 组合一次 scenario solve 与直接 before/after compare，并返回 scenario ref、单一 provenance-bound comparison ref 和最多 10 个重点城市。`prepare_network_coverage_map` 要求结果中存在的确切 `service_target_hours`，发布 `network_coverage_geojson.v2`，在需求点和末程线上携带 `attained/missed/unassigned` 状态；`prepare_network_comparison_map` 同样要求 comparison 中的确切目标，发布 `network_comparison_geojson.v3`，需求点按 after 状态、before/after 末程线按各自状态携带同一枚举；Maps 只按 typed 状态生成红绿灰图层，不重算 SLA。 | Data/Network 必须继续保持单一 provider ResourceStore 与 strict typed refs，不能恢复旧 Case 状态、Demo 入口或 Platform workflow；数据缺口不得冒充零影响，普通文字不得冒充输入卡片 | 路线/cost 由 pair/lane fact 自主部分复用；child 把 typed `needs_input` 交 Root，只有 Root 调用 Runtime 原生 `request_user_input`；模型不再选择重复的 plan/build/validate、standalone scenario 或四引用交付拼装路径 | Network Tool 拥有算法、pair facts、缺口终态与业务地图状态；Root Skill 拥有输入卡片调用；Maps 拥有展示样式；Provider ResourceStore 拥有中间内容和统一 GeoJSON ref/profile | Planner 193/193、Maps 43/43、stdio 双门、两个 Copilot 与 8 个 Skill validate、Web Rust 158 passed/8 ignored、Vitest 1257/1257、deterministic multi-agent 2/2、2026-08-24 三轮地图复验及 2026-08-27 Jakarta 50/50 真实导航通过 |
...
| `B-BROWSER-LEGACY` | 已删除 | 生产只有 `browser-entry.ts`→`WebApp`；Atom 1 已删除 generation/generic RPC/remembered approval，Atom 2a 已删除 Profile content/Agents/Prompts，Atom 2b 已删除 Workspace Preferences，Atom 3 已删除独立 Terminal，Atom 4 已删除本地 Usage | 继续维护第二 UI/API 表面会让 Platform 与 Runtime/Runner owner 冲突 | 不建替代子系统或兼容 facade | Browser 只保留 WebApp typed surface；Server 路由按 production reachability 收敛 | 已完成 |
| `B-FAKE-PRODUCTION` | 已删除 | Atom 5 删除 `--codex-mode`/`CODEX_MODE`、`--fake` 和 Server/launcher/deploy 的 Fake adapter/provider、demo Workspace、空 Copilot/delivery 分支 | 以 Fake 启动产品会绕开真实 Runtime、密文 Provider 与 Copilot source 的生产合同 | Fake 只在 Rust 测试中显式构造；生产始终启动 Profile Host、RealCodexAdapter、SecuredProviderService 与真实 Copilot/delivery source | Codex Runtime 与 Profile Host 拥有真实执行；测试直接拥有 Fake fixture | 已完成；无 Browser/API/schema 合同变化 |
| `B-PROVIDER-OWNER` | 已完成 Atom A/B | Provider definition/model catalog DB mirror、Task pair、global default、normal-turn override 与 catalog mutation 已删除；Runtime settings update 只允许 same-provider model | 从 Profile/Task/Run回填 pair 或跨 Provider 改旧 Thread 会再次建立第二 owner | Runtime/Profile config 拥有定义、目录、Profile default 与 materialized Thread settings；跨 Provider 只能新 Thread | Platform 只留密文 Secret、授权、operation lock、browser typed projection 和 new-Thread orchestration | migration 70/71、adapter/route/Browser focused gates覆盖；真实 disposable PG 门待环境提供 |
| `B-THREAD-HISTORY` | C1/C2 代码收敛 | 普通 Browser send 生成 required、bounded `clientUserMessageId`；Server 只将它写入 typed `TurnOptions`，Real adapter 写入 official `turn/start`，Fake 精确保留它。Runtime canonical `userMessage.clientId` 经安全 history projection 到 Browser；`routes/threads.rs` 不再查询 `run_events` 覆盖 Agent text、按 Tool/message 猜位置或注入 synthetic `platformApproval`。完整 Profile/app-server 重启后的第一次 Root/child read、turn list 或 send 会先从 Task 的 `copilot_package_id` 解析同一 trusted package execution config，再绑定 Thread；Runtime 因而可按原生 Role 恢复 MCP inventory，Platform 不保存第二份 Role/Tool 状态。Artifact 仍只按 exact producer `item_id` 附着，approval/user input 保持独立 Platform resource/queue | 文本去重会吞掉连续相同消息；overlay 会把 Platform event 冒充 Codex transcript；无 package execution 的冷 bind 会让 Runtime 无法恢复可信 Role MCP scope | Web 只按 official Item ID 或相同非空 client identity 合并 optimistic/live/history；Platform 不复制 Thread/Turn/Item，只做授权、首次 bind 输入和 exact Item Artifact 投影。Run 终态与 bounded event 另由后续原子收敛 | Codex Runtime 拥有 Thread/Turn/Item/history 与 Role 恢复；Platform 拥有 approval、Artifact、授权、Task package selection 与有界投影 | Adapter 18 passed、3 个外部 Runtime/FastMCP 条件门 ignored；2026-08-27 完整重启后同一 Network child 恢复并直接完成 Jakarta 精确单路线复验；no-desktop 通过 |
| `B-RUN-LIFECYCLE` | C3 代码收敛，PostgreSQL lifecycle gate 待执行 | terminal Thread follow-up 创建有 direct `continued_from_run_id` 的新 Run，而不重开旧 Run；同一 client identity 以 stable hashed Run idempotency key、direct source 和 Run-stored last exact Turn 绑定，exact retry 不会重复进入 Runtime。新 attempt 在 exact Turn response 或 official `turn/started` bind 前是 `provisioning`，两者任一先到都原子进入 `running`；adapter start 失败写成 terminal `turn_start_failed`。只有 exact `(run, thread, turn)` 的官方 `turn/completed` event 才改变 Run/Task，延迟旧 Turn 和任何 Thread terminal 不可覆盖新 attempt。interrupt 只请求 Runtime，archive 只归档 Runtime Thread；archived Task 不能创建 follow-up，non-archived Task 可从 cancelled/failed/completed Run 继续 | 同一 Thread 跨多个 Run 使没有 exact Turn identity 的生命周期事件无法安全归属；将 Thread/history projection 当作 Run terminal writer 会重现旧 attempt 覆盖 | RunOrchestrator 拥有 attempt acceptance、Turn binding 和 start-failure terminal；event projection 只消费 official event，所有 Thread terminal 只结束 Agent projection；routes 不直接写 Run/Task lifecycle | Platform 拥有 Run/Task 调度状态与有界投影；Codex Runtime 拥有 Thread/Turn/interrupt/archive 语义 | run-orchestrator pure owner tests、Server 156 passed（7 ignored）编译通过；忽略的 disposable PostgreSQL gate 覆盖 idempotency、concurrency、active conflict、old Run preservation、late event、interrupt/archive 与 failure terminal，待环境提供 |
| `B-EVENT-BOUND` | C4 已验证 | `run_events.sequence` 随 live/replay 进入 Browser，按 `(runId, sequence)` 单调去重；同文本不同递增 sequence 保留。Server 只投影当前 UI 的 lifecycle、approval、artifact、item/Tool、agent、token 与显式 turn 更新；unknown Runtime method 只持久 `codex.unknown` 的 bounded sourceType/audit marker，Browser 不映射它。MCP approval request 使用独立 allowlist，只保留安全 thread/turn/item ID、`form/url` mode 与精确 `credentialKind=maps`，URL、server name、说明、Secret 和路径全部丢弃。超过 128KiB 的 raw frame 在 JSON parse 前固定拒绝；其余 raw Runtime params 在任何 sanitize/project helper 前，以及 Browser parse、owner projection 后均受 64KiB、depth 12、object keys 64、array 64、string 12k 限制，越界成为 typed `platform.projection.bounded` | 无 sequence 的浏览器 event、文本/时间窗 dedup、未知 method 或超大嵌套 payload 会制造重复、内容泄漏或 UI 注入；通用 sanitizer 会把业务所需的 typed 凭据类型一起替换 | Event projection 拥有 method-specific allowlist、边界和 unknown audit；Browser transport 拥有 fail-closed parse 与 sequence cursor；不新建 payload codec、表或 cache | Platform 拥有有界 durable event projection；Codex Runtime 仍拥有 raw event/history | event projection 41 passed；full Web Rust、Vitest、typecheck、lint、no-desktop 与 build 既有门通过；disposable PostgreSQL 环境未提供 |
...
7. Atom A/B 已删除 `profile_provider_definitions`、目录 overlay、启动回放、Task pair、
   `models.default_selection` 和 normal-turn Provider/model override：Provider 定义、模型目录、Profile
   default 与 materialized Thread settings 都只来自 Codex Runtime。same-provider model 变更经过无 override
   resume 读/确认；跨 Provider 只新建 Thread，旧 Thread 不变。
8. C2 删除 Server approval history overlay、Agent text `run_events` 覆盖和 Browser 同文本 optimistic
   去重。Browser 的 required `clientUserMessageId` 经 Platform typed request、`TurnOptions` 和 official
   `turn/start` 写入，canonical `userMessage.clientId` 回到 history 后只与相同 nonempty client identity
   或 official Item ID 合并；approval 和 request-user-input 继续是独立 Platform resource/queue。
...
approval，Atom 2a 已删除 Profile content/Agent/Prompt 二次系统，Atom 2b 已删除 Browser Workspace
Preferences、假 Runtime 参数和 setup/launch 脚本链，Atom 3 已删除独立 Terminal，Atom 4 已删除本地
Usage，Atom 5 已删除 Fake 生产运行模式，没有增加替代缓存、兼容 facade 或新的持久状态。
旧跨 owner 数据面与 compatibility island 已删，Run/Thread 与 history/event 已按 canonical owner
收敛；Codex 已独立同步到 `76d98a771e6c`，七个 retained seam 的 Runtime、schema、app-server、
deterministic Web 和本次 DeepSeek Flash 产品门均通过。official upstream 后续推进必须另开同步切片，
不能混入这份已冻结的验收基线。
