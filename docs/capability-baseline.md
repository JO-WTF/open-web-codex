...
| Enterprise Supervisor Policy | built-in `1.7.0` exact role/capability isolation verified; user Release publication DB-verified | The catalog accepts either a reviewed repository Package or an immutable organization-scoped user Release. Supervisor Studio can save a draft, validate exact Agent and Artifact handoffs, publish a version and expose it through the same generic Run selector. Browser requests do not contain Runtime Role names, MCP inventory or Runtime capability claims; the server derives those facts from selected code-published Agent Definitions. Published Releases retain exact spec, content hash and release identity, and worker preflight re-resolves that identity before Thread creation. The real built-in journey still proves that request-scoped root configuration selects only Definition-declared capability roots, disables ordinary Apps/Plugins/shell, supplies an exact Role allowlist and one resident instance per required Role, while child Role files disable V2 delegation. The root has no business MCP tools; Role files remain Profile Host materialized, hash verified and absent from persistent Profile/Project Agent configuration. Disposable PostgreSQL tests cover fresh migration, publish/resolve and cross-Organization denial. A real Runtime smoke of a user-authored Release remains pending |
| Enterprise Agent Definitions | Data `1.6.0`, Network `1.5.0` verified | Code-managed Definitions and reviewed instructions now have one publication source under `capabilities/agents/`; the typed catalog exposes responsibilities, input/output Artifact types, required capabilities, owning capability-root IDs, risks and exact Runtime Role references. Generated Role TOML disables shell, ordinary Apps/Plugins, V2 and child delegation, then re-enables only the declared plugin MCP server with an exact `enabled_tools` allowlist. Supervisor Releases may reference only exact published versions, and only bound preflight materializes the Roles without polluting the Profile Agent catalog. Existing Profile Agent CRUD is separate Runtime configuration and is not yet publishable as a governed Agent Definition; user Agent Definition versioning, capability contracts and release governance remain unsupported |
...
| MCP | root/Role isolation verified in real Runtime | Real-mode startup prepares and import-checks the dedicated supply-chain MCP environment. `supply_chain_data.list_planning_sources` exposes bounded typed source metadata without paths or raw rows. Exact server/tool requirements and capability-root IDs are derived from selected Agent Definitions and sealed into immutable Runtime Role configuration. There is deliberately no threadless Profile inventory preflight: official `mcpServerStatus/list` cannot see Workspace/project or selected capability-root servers until a Thread owns that effective configuration. The root request exposes only the selected supply-chain capability root while disabling its business servers; after creation a thread-scoped inventory check fails closed and archives the undisclosed Thread if any restricted business tool remains visible. Child Role configuration re-enables only its declared server/tool allowlist. The real `1.7.0` run recorded no Root MCP/command calls; Data used only its data operations, while Network used generic Resource discovery/read plus planner operations and no Data business operation. No `map_utils` call appeared. General OAuth/full-form elicitation and MCP Studio lifecycle remain unsupported |
...
| Persistence | PostgreSQL migrations cover users/sessions, organizations/memberships, Profiles/capabilities/encrypted Secrets, projects, tasks, Runs, leases, Workspaces, durable approvals/audit, Supervisor Definitions/draft Revisions/immutable Releases and Policy snapshots/bindings, rebuildable Runtime Agent tree and child-task execution observations (`runtime_agent_projections`, `runtime_agent_execution_projections`), Task-owned typed Artifacts with producer provenance, and versioned Run-event projections. Release metadata is governance state; generated Runtime Role configuration is not duplicated in PostgreSQL. Disposable fresh-schema tests prove publication resolution and cross-Organization denial; existing tests also prove deleting a producer Run does not delete its Artifact, Task grant or immutable provenance identity | Release deprecation/deletion policy, concurrent edit conflict UX, user Agent Definition publication, Artifact replacement/invalidation/deletion/retention/cross-Run reuse and complete multi-user constraints remain missing |
...
| Browser | established WebApp presentation runs through typed platform resources for workspace/thread/message, approvals, Provider/model, MCP/rate-limit snapshots, files, Git status and Supervisor governance. Supervisor Studio lists server-published Agent responsibilities, Artifact inputs/outputs and required capabilities; users edit Supervisor responsibilities/instructions, select Agents and required deliverables, validate and publish. The generic Workspace action lists both built-in Packages and organization Releases and starts the exact selected version. The browser never submits Runtime Role names, MCP inventory, capability gates, local paths or release content at Run start. The existing Agent/Files sidebar continues to render root/child projections, immutable child task nodes, bounded activities, waiting states and Task Artifacts without exposing reasoning, tool arguments/results, internal Resource URIs or host paths | add user Agent Definition and capability-package lifecycle, make child Agent task nodes navigable to authoritative histories, add Artifact content/open actions, then cover Release deprecation, concurrent editing, deeper trees, failure/approval states, narrow layouts, shared-Workspace concurrency and broader cross-user denial |
