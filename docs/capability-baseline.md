...
| Provider 定义与选择 | Profile `config.toml` 和 Runtime 已能持久化 Provider/模型及未来 Thread 默认选择；已物化 Thread 的实际 pair 由 official Thread settings 拥有。Runtime Provider catalog 必填返回 `supportsFunctionTools`；Platform 在 `profile_provider_definitions.supports_function_tools` 精确持久化并向 Web 投影，Chat 表单显式编辑且 Responses 隐藏保留。关闭时 Runtime 返回 `providerFunctionToolsUnsupported`，Platform 只投影固定安全消息；模型级 `supportsSearchTool` 保持独立。Platform 仍镜像 Provider 定义与 global `models.default_selection`，并在启动时回放 | 第二/第三 owner 与 Task pair 全量收敛进入后续 backlog；当前能力字段、错误类型和持久化链已通过 Runtime/Platform/Web/重启与真实 Turn 验收 |
...
| `P1-SC-DATA4` | 当前关键链 | `data_server.py` active path 的 inspect 返回 bounded inline `source_profile`、`preview_sample_count`、`total_count`、`total_count_exact`、`inspection_identity` 与 `inspected_relative_paths`；identity 按排序 exact relative path 和每个完整 regular-file SHA-256 canonical hash。`prepare_network_input` 重新 inspect 并校验 identity，变化返回 `source_inspection_changed` 且不写 output；confirmed business sources 与可选行政区路径必须来自 inspected set，inline profile 只用于当前 Data mapping | 预览样本曾被模型误认为完整正文，且生成输入曾落在 Workspace 根目录或源数据目录；旧 source profile Resource 使 Data/Network 产生不必要的 read 链 | strict surface：discover→一次 inspect→inline identity/profile→一次 prepare；Data `list_resources`/templates 为空；目录截断时不得断言候选仓不存在；Indonesia built-in asset 只属于验收 fixture，其他国家无 provider 时 typed unavailable | Data Tool 拥有格式/映射/inspection identity/标准化和生成目录；复用 `workspace_intake.py`、`mapping.py`、`normalization.py` 中纯函数与 `geography.py`，不建 source/revision/CAS 包装 | focused Data tests 覆盖 sample/total、order-independent identity、byte/path/set/symlink 变化拒绝、Balikpapan 位于 preview 外完整准备、Data Resource surface empty 与两套 stdio domain gates |
