...
| --- | --- | --- | --- | --- | --- | --- |
| `P1-R2-CHAT` | 当前关键链 | pinned official Codex 无 Chat transport 和当前 Platform `PUT/list` 所需的 `modelProvider/list`；当前窄 seam 位于 `codex/codex-rs/codex-api/src/chat_translate.rs`、`codex/codex-rs/core/src/{session/turn_context.rs,tools/router.rs,tools/handlers/tool_search.rs}`、`codex/codex-rs/core/src/client/chat.rs` 与 app-server catalog processor，Platform Secret 位于 `apps/web/crates/provider-service`。Chat bridge 将 Runtime 原生 client ToolSearch 映射为普通 function call；成功结果仅登记进当前 Turn 的 plan，所有 sampling Router 为 Responses 与 Chat 生成同一 `Prompt.tools`，Chat 不扫描历史 ToolSearch output 补载 schema；Role MCP server scope 仍为搜索边界 | 历史 loaded schema 若进入新 Turn，会绕过原生重新发现并造成旧 provider schema 与当前 Runtime registry 漂移；真实 DeepSeek 也可能无法完成 tool-search 双轮，或 Secret 穿透到普通配置/日志 | 只保留真实仓网 gate 的最小 typed seam：`PUT/list`、密文 Secret restart、显式 `deepseek-v4-flash` 当前 Turn ToolSearch→延迟 MCP Tool→final；model refresh/catalog 不在当前 gate | Codex 拥有 wire/app-server typed 执行、deferred registry 与搜索，Platform 只拥有 CRUD 授权与密文注入；以真实 gate、跨 Turn loaded-schema 回归与零明文负向门为删改条件 | 已通过：Chat request/stream/history round-trip、跨 Turn 不重放 loaded schema 的 chat translator 回归、Core direct/deferred gate、Role config projection，以及真实 DeepSeek `tool_search`→加载 `evaluate_network_baseline`→授权 MCP 成功→final；缺失实际分配关系的 `actual_current` 输入也返回了业务 typed error，未退化为传输失败 |
