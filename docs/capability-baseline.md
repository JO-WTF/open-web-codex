...
| MCP | reviewed declarations plus bounded Python package authoring; Runtime execution verified | Agent Studio lists reviewed MCP declarations separately from current Thread status. Its Python editor accepts bounded Tool JSON Schemas and standard-library functions, generates a fixed stdio MCP launcher, clears inherited environment, validates startup/discovery, can test one declared Tool, and atomically publishes an immutable Workspace package. A real new stock-history Thread recorded the generated Server becoming ready and calling `lookup_stock` followed by `get_price_history`; this proves selected-root Runtime MCP execution, not general MCP lifecycle. Supply-chain root/Role isolation remains verified: Root has no business MCP, while each child Role receives only its exact allowlist. Secret-backed servers, OAuth/full-form elicitation, arbitrary commands/transports and general MCP config/reload/delete remain unsupported |
...
| Browser | established WebApp presentation runs through typed platform resources for workspace/thread/message, approvals, Provider/model, MCP/rate-limit snapshots, files, Git status and Agent/Supervisor governance. Agent Studio provides a bounded Python capability editor for Tool Schema, standard-library implementation and Skill instructions; users can validate MCP startup/discovery, test a declared Tool, atomically publish to an authorized Workspace and start a new test Thread without supplying a path, launcher or environment. Agent Studio also provides a platform-Owner editor for immutable Supervisor behavior contracts, Agent Catalog authoring from reviewed templates, and Supervisor authoring with exact Agent Releases and Artifact handoffs. Published details display platform and custom instruction sections separately with content and execution hashes. The browser never submits Runtime Role names, MCP inventory, Tool allowlists, capability roots, local paths or release content at Run start. Agent task nodes open Codex-owned child Thread histories through a Run-scoped authorized route, and ready Artifact cards open content through the existing Task-grant-protected Artifact URL. The Agent/Files sidebar continues to render bounded Runtime projections without exposing reasoning, Tool arguments/results, internal Resource URIs or host paths | add native Agent/Skill/Plugin/MCP lifecycle and Secret-backed package authoring, then cover Release deprecation, concurrent editing, deeper trees, Artifact replacement/invalidation/deletion/retention, partial-failure and approval-rejection recovery, narrow layouts, shared-Workspace concurrency and broader cross-user denial |
