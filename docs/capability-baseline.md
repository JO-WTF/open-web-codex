...
| Provider 定义与选择 | Profile `config.toml` 和 Runtime 唯一持久化 Provider/模型及未来 Thread 默认选择；已物化 Thread 的实际 pair 由 official Thread settings 拥有。Runtime Provider catalog 必填返回 `supportsFunctionTools`、模型级 `supportsSearchTool` 和 nullable `currentModelId`；任一缺字段由 Platform 拒绝。`profile_provider_definitions` 镜像与启动回放已删除，含旧数据的开发库由 migration 70 拒绝并要求显式重建。Platform 对可编辑 Provider 保留产品声明：Chat 写 true、Responses 写 false；Web 不展示 Function tools 复选框，且这不构成 Runtime transport 推断。`models.default_selection` 暂只供下一原子的 Task seed，不能覆盖 catalog 或 Thread | Task seed/global default 的剩余 owner 收敛进入下一原子；当前交互只保留 Wire API 与模型级 Tool search 两个独立选择面 |
...
| `B-PROVIDER-OWNER` | 当前 Atom A 后 | `profile_provider_definitions`、目录 overlay、启动回放和 catalog 手工 mutation 已删除；`models.default_selection` 仍暂存为下一原子的 Task seed | 若 global default 继续改写 Runtime catalog 或已物化 Thread，会再次形成第二 owner | Runtime/Profile config 已唯一拥有 Provider 定义、模型目录、catalog 和未来 Thread 默认；下一原子只收敛 Task seed/global default，不触碰 Thread actual pair | Platform 只留密文 Secret、授权、operation lock 与必要 audit；Codex Runtime 拥有配置/目录/选择 | Atom A 已实现；Task/default 原子待后续 |
| `B-THREAD-HISTORY` | Provider Task/default seed owner 后 | `codex-adapter/src/real.rs` 的 history mode、`routes/threads.rs` 的 overlay、`WebApp.tsx`/`webThreadHistory.ts` 的 live/history 启发式合并 | 连续同文本消息误去重，Task/Run status 可以冒充 official Thread/Turn 事实 | hidden generation 已删除；Provider Task/default seed 收敛后，再按 official identity 处理 Run/Thread 与 history | Codex Runtime 拥有 Thread/Turn/Item/history，Platform 只做授权和 exact Item 投影 | 已证实，待后续 Atom；2026-08-23 本地 PostgreSQL ignored 跨租户门在既有 completed Thread follow-up 请求处返回 `502` 而非 `200`，Preferences 与 Terminal fresh-schema retired-table 门均独立通过，Atom 2b/3 已验证未掩盖该失败；Atom 4 未触及此路径 |
...
7. Atom A 已删除 `profile_provider_definitions`、目录 overlay 与启动回放：Provider 定义、模型目录和
   catalog 当前字段只来自 Codex Profile config/Runtime。`models.default_selection` 仍是下一原子待删的
   Task seed；它不得覆盖 catalog 或后续 Turn，非当前 Provider refresh 只对成功的 Runtime 写入安排安全
   refresh，不能切换 current Provider。
...
后续顺序固定为 Provider/model Task/default seed owner → Run/Thread 与 history/event；
