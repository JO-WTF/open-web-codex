...
| `B-THREAD-HISTORY` | C1/C2 代码收敛 | 普通 Browser send 生成 required、bounded `clientUserMessageId`；Server 只将它写入 typed `TurnOptions`，Real adapter 写入 official `turn/start`，Fake 精确保留它。Runtime canonical `userMessage.clientId` 经安全 history projection 到 Browser；`routes/threads.rs` 不再查询 `run_events` 覆盖 Agent text、按 Tool/message 猜位置或注入 synthetic `platformApproval`。Artifact 仍只按 exact producer `item_id` 附着，approval/user input 保持独立 Platform resource/queue | 文本去重会吞掉连续相同消息；overlay 会把 Platform event 冒充 Codex transcript | Web 只按 official Item ID 或相同非空 client identity 合并 optimistic/live/history；Platform 不复制 Thread/Turn/Item，只做授权和 exact Item Artifact 投影。Run 终态与 bounded event 另由后续原子收敛 | Codex Runtime 拥有 Thread/Turn/Item/history；Platform 拥有 approval、Artifact、授权与有界投影 | full Web Rust 156 passed（7 个环境标记 ignored）、Vitest 174 files/1255 tests、lint/typecheck/no-desktop/build 通过；disposable PostgreSQL security gate 仍待环境提供 |
...
| `B-EVENT-BOUND` | C4 已验证 | `run_events.sequence` 随 live/replay 进入 Browser，按 `(runId, sequence)` 单调去重；同文本不同递增 sequence 保留。Server 只投影当前 UI 的 lifecycle、approval、artifact、item/Tool、agent、token 与显式 turn 更新；unknown Runtime method 只持久 `codex.unknown` 的 bounded sourceType/audit marker，Browser 不映射它。超过 128KiB 的 raw frame 在 JSON parse 前固定拒绝；其余 raw Runtime params 在任何 sanitize/project helper 前，以及 Browser parse、owner projection 后均受 64KiB、depth 12、object keys 64、array 64、string 12k 限制，越界成为 typed `platform.projection.bounded` | 无 sequence 的浏览器 event、文本/时间窗 dedup、未知 method 或超大嵌套 payload 会制造重复、内容泄漏或 UI 注入 | Event projection 拥有 allowlist、边界和 unknown audit；Browser transport 拥有 fail-closed parse 与 sequence cursor；不新建 payload codec、表或 cache | Platform 拥有有界 durable event projection；Codex Runtime 仍拥有 raw event/history | full Web Rust 158 passed（7 ignored）、Vitest 174 files/1256 tests、typecheck、lint、no-desktop 与 build 通过；disposable PostgreSQL 环境未提供 |
