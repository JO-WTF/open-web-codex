...
| `P1-SC-NET9` | 当前关键链 | `server.py` 的 14 个 decorated active Resource/final tools 已使用 strict `ResourceRef`，9 个未注册 Case lifecycle entry wrappers 与 4 个 legacy Case Skills 已删除；Data Resource 保留完整 provided route facts，Network 可按 scope 物化路线矩阵、确定性返回双覆盖口径，并从 exact comparison 产生地图卡片数据与 Markdown 简报；`ArtifactRef | DataAgentRef`、`_case_from_input_ref`、`CaseRepository` 及 mapping/matrix/analysis/scenario service 仍在 non-active compatibility island | Data4 输出的 active path 已可用单一 ref，剩余 Case/ArtifactRef tail 仍形成第二业务状态和维护面 | 保持 current decorated strict `ResourceRef` surface；路线/cost 由 pair/lane fact 自主部分复用，Stage E 原子删除 Case/ArtifactRef compatibility tail | Network Tool 拥有算法、pair facts 与业务渲染；保留 `matrix.py`/`solver.py` 纯算法，删 `case_repository.py` 及 Case service/schema/tests | Active surface landed，provided facts/双覆盖口径的 Python 与真实 stdio 门通过；9 个 entry wrappers 与 4 个 legacy Case Skills 已删除，CaseRepository、ArtifactRef、NetworkSnapshot 及其它 non-active tail 保留至 Stage E |
