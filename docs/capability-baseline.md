...
| `B-THREAD-HISTORY` | Provider owner 后 | `codex-adapter/src/real.rs` 的 history mode、`routes/threads.rs` 的 overlay、`WebApp.tsx`/`webThreadHistory.ts` 的 live/history 启发式合并 | 连续同文本消息误去重，Task/Run status 可以冒充 official Thread/Turn 事实 | Provider pair 已不再成为 Task/Turn/history side channel；后续按 official identity 处理 Run/Thread 与 history | Codex Runtime 拥有 Thread/Turn/Item/history，Platform 只做授权和 exact Item 投影 | 已证实，待后续 Atom；2026-08-23 本地 PostgreSQL ignored 跨租户门在既有 completed Thread follow-up 请求处返回 `502` 而非 `200`，Preferences 与 Terminal fresh-schema retired-table 门均独立通过，Atom 2b/3 已验证未掩盖该失败；Atom 4 未触及此路径 |
