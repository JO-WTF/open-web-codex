...
| Runtime Role | 两个企业 Definition 映射到精确 Runtime Role；启动用正式 `config/read` 校验，真实 spawn 已证明二者可发现 | 独立的 Runtime 可用性/企业发布审计视图尚未完成 | 由 Codex Profile 正式配置和发现，不由平台隐藏安装 |
...
- [x] 对应 Runtime Role 通过 Codex 正式配置与发现路径进入临时单 Profile；
- [x] 平台启动 Task 时校验 Definition 已发布，并通过 Codex 正式
  `config/read` 确认精确 Runtime Role 已配置；
- [x] 用真实 spawn 证明两个已配置 Runtime Role 均可被当前 Runtime 发现；
- [x] Role 未配置时明确失败，不回退到 default 或其他 Agent；
- [ ] Profile Runtime Role 配置与企业 Definition 分开显示和审计；
- [ ] 不把 Runtime nickname、AgentPath 或 Role 名称作为企业数据授权身份。

现有 Profile Agent 设置可以继续服务 Runtime 配置，但不能被描述为完整企业 Agent
Catalog。平台也不能在启动脚本中直接修改隐藏 Profile 文件来伪造 Role discovery。
