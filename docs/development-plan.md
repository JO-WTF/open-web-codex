...
| M0-B03 | P0/M | [-] | 从 experimental annotation 生成能力实验状态 | 注册表与 annotation 表对照、stable Schema 排除实验 wire 名测试已落地；stable TS 过滤已覆盖 Notification；声明字段全自动生成仍待完成 |
| M0-B04 | P0/M | [-] | 定义能力 ID 与产品方法归属 policy | `manifest_method_policy` 与 `policy/feature-policy.v1.json` 已建立；完整 Feature Policy 单测覆盖待 M0-C05 |
...
| M0-C02 | P0/M | [-] | 将 `apps/web/contracts/codex` 分成 generated bundle 与 product policy | `policy/` 与 `generated/` 目录已建立；`check:codex-contracts` 已校验 feature policy 与 bundle |
| M0-C03 | P0/M | [ ] | 从 Bundle 生成 Rust/TypeScript 消费类型 | 生成物不可手改检查 |
| M0-C04 | P0/S | [-] | 以 SHA-256 缓存并固定 Bundle | `fetch-codex-contracts` 缓存与校验已实现；CI 固定摘要仍待补 |
| M0-C05 | P0/M | [-] | Product Feature Policy 映射 UI 功能到能力/最低版本 | `policy/feature-policy.v1.json` 已建立；单元测试覆盖所有 P0 功能仍待完成 |
...
1. 完成 `M0-B03/B04` 剩余项：全自动 experimental 声明生成、Feature Policy 单测覆盖。
2. 完成 `M0-B09` 与 `M0-C03/C04/C05`：CI 固定 bundle 摘要、从 Bundle 生成消费类型。
3. 保留真实 `--require-manifest` Smoke，并增加 Schema/Manifest 漂移门禁。
4. Legacy `/api/rpc` 与 permissive CORS 已默认关闭；本地迁移需 `CODEX_ALLOW_LEGACY_PROXY=1`。
