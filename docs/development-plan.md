...
| M0-C06 | P0/M | [x] | 真实 initialize + Manifest Smoke | 本地构建返回 17 个声明 |
...
完成上述三批后再进入 M2 多用户 Beta。M2 的首个门禁是两用户跨 Profile、Thread、Workspace、事件、审批和 Secret 的系统性拒绝矩阵，而不是先增加更多页面。
