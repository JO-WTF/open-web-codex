...
| --- | --- |
| 计划基线 | 2026-07-14 |
| 当前里程碑 | M1-D Git Runtime、Runner 与 Task 纵向编排 |
| 默认分支 | `main` |
| Web 目录 | `apps/web` |
| Codex 目录 | `codex` |
...
| M0-C08 | P0/M | [ ] | Provider 切换、force refresh、Turn Provider Smoke | 双 Provider 切换/refresh/缓存隔离真实 Smoke 已通过；Turn Provider 传播仍待补 |
...
- 一次性 bootstrap、Argon2id 密码 Session、Organization 切换、`/me`，以及组织与成员、项目、Task、Run 的 API。
- Fake/Real `CodexAdapter`、Run start/cancel/message、Runtime event fan-out、Run terminal event 持久化和 Task 状态联动。
- 类型化 Provider DTO/service/认证路由和浏览器客户端；AES-GCM Secret Store 只向 Profile 子进程注入明文，真实 app-server Smoke 验证配置不含密钥。
- PostgreSQL migration 覆盖 Organization-bound Session、Profile/Capability/Secret、Workspace、Approval/Audit 和资源 `organization_id`；空库与重复启动测试通过。
- 命令、文件和权限审批先持久化再投影，决策使用版本 CAS，成功或未知派发结果与审计原子写入。

这些实现尚不能按 M1 完成：Session 仍允许 Bearer，完整 Cookie/CSRF/logout/revoke/限流未完成；Lease、Artifact、Job 和幂等键服务缺失；Run 还没有 Mirror/Worktree/Runner 编排；审批过期、Profile 重启后的未知派发恢复和结构化输入尚未闭环；浏览器其他域仍使用本地 RPC/SSE MVP。后续任务必须基于这些现有模块增量完成，不能另建平行 Server 或第二套 Task/Thread 模型。
...
| M1-E10 | P0/M | [x] pending Approval 落库后再向 Web 推送 | 崩溃窗口与安全投影测试 |
| M1-E11 | P0/M | [x] Approval 决策 CAS 和 Codex 响应 | 并发决策只有一个成功；过期/未知派发恢复仍归 M1-E12 |
...
### Batch 2：Profile Host 与持久审批最小闭环（已完成当前范围）
...
2. [x] Provider CRUD/模型刷新进入平台 service，Tauri 只复用兼容入口；Server 直接凭据进入加密 Secret Store 和 Profile 子进程环境。
3. [x] 增加 Profile/Capability/Secret/Workspace/Approval/Audit migration 和 Organization/Profile 归属约束。
4. [x] 命令、文件和权限审批先持久化再发送无 request-id 的安全投影，决策使用版本 CAS 并写审计。
5. [x] 真实 Profile restart、Thread resume/read、Provider switch/cache 和加密 Secret 注入/删除 Smoke 通过。

**完成证据：** Host 原位重启恢复同一用户 Thread；双组织回归拒绝跨 ID、Profile 和审批访问；Codex config 不含直接凭据。审批过期和 Profile 重启后的 `delivery_unknown` 运维恢复归入实时层剩余任务。
