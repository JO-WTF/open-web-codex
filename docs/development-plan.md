...
10. 当前迁移删除 `browser_workspace_preferences` 表；不读取、迁移或删除任何用户 Profile 文件。
    Terminal owner 已由随后 Atom 3 删除，Usage owner 已由 Atom 4 删除。
...
PostgreSQL fresh-schema retired-table 门已通过；C1 令 ignored 跨租户 fixture 显式 seed 同一授权
Workspace 的 persisted Fake Thread，不再以“找不到 Fake Thread”制造 502。该完整门仍需 disposable
PostgreSQL 环境复验；没有用 Preferences fallback、重试或放宽断言掩盖任何 Runtime 错误。
...
13. fresh-schema retired-table gate 和认证 404 route assertion 覆盖删除；不为同 Workspace 多 Task
    或 fork 新建 Terminal→Run selector。

Atom 4 已完成：

14. 删除 `/profile/usage` 的 run_events 累计 token/Turn 聚合、空数据时的 `account/usage` fallback、
    `LocalUsage` contract/client 与 `ProfileQuery::Usage`；不新增或修改数据库 schema。
15. 删除只消费该 API 的 Home 使用量卡片、图表、工作区筛选、刷新、设置和 tray 回调；Home 的
...
16. 认证 404 assertion 与 active-source rg 覆盖删除；Browser legacy API 至此清完。Fake 部署模式不属于
    Browser legacy，已由随后 Atom 5 处理；completed Thread follow-up 只通过 exact fixture binding
    复现，不引入路由 fallback。
...
19. 无 Browser/API/schema 合同变化；生产冷启动缺外部服务时明确失败，不回退 Fake。测试 Fake 的
    completed Thread 只能由显式、Workspace-bound fixture seed 建立，不能由 send 自动创建。
...
2026-08-21 ADR-024 最终复跑中，多 Agent 业务门为 40 个有效 Chat 轮次、133.552 秒；单 Agent
业务门为 26 个有效 Chat 轮次、109.585 秒，并完成 580 条报价 evidence 绑定与 90.0521% 需求加权
覆盖。旧门对应基线为 Multi 26 轮/91.768 秒、Single 24 轮/84.851 秒，第一批拆门基线为 Multi
26 轮/98.407 秒、Single 23 轮/97.383 秒。Data 标准链已缩为 discover→inspect→prepare，单设施
路线已精确到候选 ID，最小仓数已收敛为最多两个 CP-SAT stage，共享 Tool build store 也从约
...
成功后安排安全 refresh；Provider 定义/模型目录数据库镜像、Task/default seed 和 normal Turn Provider
override 已删除。已物化 Thread 只允许 same-provider 官方 settings model 更新；跨 Provider 必须新 Thread。Browser dead graph 仍不属于阶段一关键门。
...
Task creation selection、Run lease/bounded event 与
