...
当前已完成两个项目的 monorepo 合并、官方上游跟踪工具、根文档和基础 Web/合同 CI。当前 Codex 定制分支基于官方共同基线 `f959e7f`，定制提交 38 个，观察到的官方 `main` 在共同基线之后有 336 个提交待集成。真实 app-server 可以完成 `initialize`，但尚未输出 Capability Manifest。
...
| Platform | `apps/web/src-tauri`（过渡）及待建 server/crates | PostgreSQL、API、Profile Host、Runner、Git、安全 |
...
| M0-A02 | P0/S | [x] | 建立 `openai/codex` 状态与 guarded sync 脚本 | 状态命令报告 336/38，默认不改文件 |
| M0-A03 | P0/S | [x] | 建立根 AGENTS、CI、架构与同步规则 | GitHub CI 通过 |
| M0-A04 | P0/S | [x] | 将旧产品/计划文档改为 canonical 根文档入口 | 仓库内只有一套产品/里程碑事实 |
| M0-A05 | P0/M | [ ] | 审查 CodexMonitor `main` 独有 Web parser/auth 测试 | 决定重放或记录已被替代 |
| M0-A06 | P0/M | [ ] | 审查 Codex Fork `main` 独有 Provider WIP | 提取 open-codex 缺失修复，避免重复实现 |
| M0-A07 | P0/L | [ ] | 创建官方同步分支并合并选定 `openai/codex` checkpoint | 冲突清单、分层提交、无未解决标记 |
| M0-A08 | P0/M | [ ] | 将 38 个定制提交归类为 upstreamed/retain/drop/rewrite | `docs/custom-codex-patch-map.md` |
| M0-A09 | P0/M | [ ] | 重点重验 Provider Wire API、模型缓存和当前 Provider 传播 | scoped Rust tests 通过 |
| M0-A10 | P1/S | [ ] | 固定首个兼容 Codex commit、Rust toolchain 和构建 target | `.sync` 与版本文件更新 |
...
| ID | P/规模 | 状态 | 任务 | 验证 |
| --- | --- | --- | --- | --- |
| M0-B01 | P0/M | [ ] | 定义 Manifest v1 Rust 类型与生成入口 | JSON Schema + roundtrip tests |
...
| M0-B04 | P0/M | [ ] | 定义能力 ID：protocol/thread/turn/approval/profile/memory/agents/skills/plugins/mcp/tools/provider | ID 唯一、版本化、所有方法有归属 |
| M0-B05 | P0/S | [ ] | Manifest 加入 build commit、version、target、protocol range | 初始化 Fixture 验证 |
| M0-B06 | P0/M | [ ] | 定义 limits 与 structured reason | unsupported/degraded/incompatible tests |
| M0-B07 | P0/M | [ ] | `initialize` 返回 Manifest 或提供初始化后只读能力方法 | 旧客户端兼容测试 |
...
| M0-C06 | P0/M | [ ] | 真实 initialize + Manifest Smoke | 当前定制二进制通过 |
...
- [ ] `G0-01` 官方同步分支完成，定制 patch map 已评审。
...
当前不应直接开始登录页或完整 Web UI。建议按以下顺序执行：

### Batch 1：官方同步准备与定制 Patch Map

1. `M0-A05/A06` 审查两个 Fork 的 main-only 提交。
2. 为 38 个 Codex 定制提交生成按模块分类的 Patch Map。
3. 记录 Provider 关键行为测试和当前 Schema，形成同步前保险网。
4. 运行 `scripts/sync-codex-upstream.sh --apply`，仅在专用分支处理官方同步。

**完成证据：** Patch Map、冲突报告、Provider/app-server scoped tests、生成 Schema 无漂移。

### Batch 2：Capability Manifest 最小纵向实现

1. 完成 `M0-B01/B02/B05`：Manifest 类型、方法集合、构建身份。
2. 完成 `M0-B07`：initialize 只读返回。
3. 先声明 `protocol.initialize`、`thread.lifecycle`、`turn.lifecycle`、`approval.lifecycle`、`provider.models` 六个 Alpha 必需能力。
4. Web Harness 消费真实 Manifest，删除“当前 Fixture 即事实”的假设。

**完成证据：** 真实二进制 `--require-manifest` Smoke 成功，Web CI 使用构建 Bundle。

### Batch 3：平台 ADR 与最小 Server Skeleton

可与 Batch 2 后半并行：

1. 决定 Web Server 与顶层 Cargo Workspace 结构。
2. 决定 PostgreSQL migration/query 方案。
3. 定义平台 API error/idempotency/event envelope。
4. 创建 Health、Config、DB migration 和 Fake Codex adapter 最小程序。

**完成证据：** 空白环境启动、Migration 重复运行、Health/错误合同测试。

只有 Batch 1–2 稳定后，才进入 Profile Host 和 Task Orchestrator。这样可以避免在 336 个上游提交尚未整合时围绕旧协议写大量适配代码。
