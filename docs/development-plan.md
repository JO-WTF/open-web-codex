...
| M0-B03 | P0/M | [-] | 从 experimental annotation 生成能力实验状态 | 注册表 `experimental_reason` 查找与 Manifest `experimental` 一致性校验已落地；stable/experimental Schema 构建对照与声明字段自动生成仍待完成 |
| M0-B04 | P0/M | [-] | 定义能力 ID 与产品方法归属 policy | `manifest_method_policy` 显式 exclude、覆盖跟踪与 policy 一致性测试已落地；Alpha 仍只承诺子集，完整 Feature Policy 映射待 M0-C05 |
...
1. 完成 `M0-B03/B04`（`M0-B02` 已完成）：从注册表/annotation 生成实验状态与产品方法归属 policy（含显式 exclude）；不要求 Manifest 列举全部 wire 方法。
