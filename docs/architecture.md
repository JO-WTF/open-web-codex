...
| Provider 定义、模型目录与 Profile 的未来 Thread 默认选择 | Codex Profile config + Runtime；Platform 保存 Browser catalog 投影 | `config/batchWrite` 持久化 Runtime 配置；Provider 的 `ModelProviderInfo.models` 是 exact `ProviderModelConfig` 列表，`ModelsManager` 按 `model_id` 将显式 `supports_search_tool` 合并到原生 `ModelInfo`，未配置默认 false，绝不按模型名或普通 `/models` ID 推断。Platform 的 Profile catalog 保存 Web 配置入口所需的 provider/model 投影、per-model `supportsSearchTool` 和非敏感 credential env-key 名称；refresh 保留既有 capability，空/失败结果不覆盖。真实 fresh Profile 已通过 provider-scoped Fetch、选择、重启恢复和新 Thread 创建，不再把空 model pair 发给 Server |
