...
| Provider 定义、模型目录与 Profile 的未来 Thread 默认选择 | Codex Profile config + Runtime；Platform 只保存密文 Secret，并传递 Browser catalog 投影 | `config/batchWrite` 持久化 Runtime 配置；`modelProvider/list` 是目录、当前 Provider 与 required-nullable `currentModelId` 的唯一返回来源。Platform 不读写或覆盖 `profile_provider_definitions`：migration 70 只允许空表删除，含旧数据的开发库必须显式重建。Runtime 的 `supports_function_tools` 与 `ModelProviderInfo.models` 都是显式 typed 配置事实，缺字段的 catalog 响应由 Platform 拒绝。Platform 对可编辑 Provider 保留单一产品规则：`wireApi=chat` 时写入 `supportsFunctionTools=true`，Responses 写入 false；这不是 Runtime 按 transport 名推断，Web 也不提供第二个 Function tools 复选框。模型级 `supportsSearchTool` 仍按 exact `model_id` 由 Runtime 配置持久化；refresh 和单模型更新只操作 Runtime 完整模型目录，成功后安排安全边界 Runtime refresh，不合并数据库目录。`models.default_selection` 暂只作为下一原子的 Task seed，不改写 catalog 的 current 字段；已物化 Thread 的实际 pair 继续由 official Thread settings 拥有。Runtime 的 `providerFunctionToolsUnsupported` 仍保护手工或外部 Runtime 配置，其他 bad request 继续使用通用脱敏文案。 |
...
shell、输出和审批保持不变。Atom 4 已删除 `/profile/usage` 的本地近似聚合、官方账户用量 fallback、
Home/tray 使用量面及其 DTO/client；Runtime Thread token、`/profile/rate-limits` 和
`provider_call_metrics` 保持。Atom 5 进一步删除 Fake 作为 Server/launcher/deploy 运行模式的分支；
Fake adapter/provider 只允许测试代码显式构造。生产组合始终是 Profile Host、RealCodexAdapter、
SecuredProviderService 和真实 Copilot/delivery source。Workspace 固定归属、
