...
- native client `tool_search` is flattened to the reserved `tool_search`
  ...
  while `tool_search` is visible, Chat requests disable parallel Tool calls so a
  Provider cannot legitimately combine discovery and an undiscovered business
  Tool in one response;
