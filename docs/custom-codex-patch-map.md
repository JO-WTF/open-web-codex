...
| --- | --- | --- | --- | --- | --- |
| `provider-chat-transport` | `codex-api/src/chat_translate.rs`, `chat_translate_history.rs`, `chat_translate_tests.rs`, `endpoint/chat.rs`, `sse/chat.rs`; minimal Chat capability consumption in `core/src/tools/spec_plan.rs`; isolated Core transport in `core/src/client/chat.rs` with minimal dispatch in `core/src/client.rs` | Translates third-party Chat Completions requests, streams, mixed/interrupted Tool calls and assistant output into Codex semantics. Responses namespaces, including MCP plugin tools, are flattened to Chat functions with request-scoped reverse mapping. The retained bridge maps native client `tool_search` into one ordinary Chat function, restores the call as `ToolSearchCall`, replays its Tool result, and adds only the current Turn's loaded schemas to that Turn's next Chat request; the Runtime still owns BM25 search, deferred registration, Tool identity, approval and dispatch. Plaintext native Agent messages retain the established assistant-side Chat projection, while collaboration `spawn_agent`, `send_message`, and `followup_task` calls carry the official direct-plaintext marker; encrypted Agent messages remain a typed preflight rejection. The checked-in stream translator currently marks text accompanying Tool calls as `commentary` and text-only completion as `final_answer`; this is a known non-standard heuristic scheduled for replacement by the phase-neutral, ordered contract in `docs/chat-responses-translation-spec.md`. | 1 | `just test -p codex-api`; focused Core direct/deferred MCP plan test; Chat request/stream/history ToolSearch round trip plus cross-Turn schema-isolation regression; real DeepSeek search → loaded MCP Tool → final smoke | Upstream provides equivalent third-party Chat ToolSearch lifecycle, including current-Turn loaded-schema replay, or the configured Chat providers expose native Responses `tool_search`. |
