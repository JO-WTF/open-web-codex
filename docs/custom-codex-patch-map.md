...
| `retain-core`: app-server Provider API | `app-server-protocol/src/protocol/{common.rs,mod.rs,v1.rs,v2/model.rs,v2/turn.rs}`, `app-server/src/{models.rs,message_processor.rs,request_processors.rs}`, `request_processors/catalog_processor.rs` | Required `modelProvider/list`, Provider-scoped models, refresh, selection, Turn-level Provider override, and capability exposure. Keep only Provider request registrations and handlers when replaying high-churn dispatch files. |
...
| `retain-core`: Provider propagation followers | `core/src/session/handlers.rs`, `exec/src/lib.rs`, `login/src/auth_env_telemetry.rs`, and their focused tests | These changes propagate the selected Provider or satisfy the expanded Provider metadata shape. They follow the owning Provider seam and are not independent feature surfaces. |
| `move-out` | `utils/home-dir/src/lib.rs` missing-`CODEX_HOME` auto-creation | Profile creation belongs to the Platform Host. Preserve the current user experience by creating the Profile home before spawning Codex, then return this utility to official validation semantics. |
| `verify before final classification` | `app-server` remote-thread/turn tests and `core` stream/header tests | Their current behavior must be traced to a retained Provider/TUI seam or an official equivalent before they are classified as `retain-core`, `upstreamed`, `move-out`, or `drop`. No deletion is authorized yet. |
