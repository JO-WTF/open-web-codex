...
| `platform-runtime-contract-fixes` | `app-server-protocol/src/{capability_manifest.rs,protocol/v2/thread.rs}`, `app-server/src/{config_manager_service.rs,request_processors/thread_processor.rs}`; `core/src/{config/mod.rs,agent/{control.rs,control/spawn.rs}}` and the narrow `thread_manager.rs` spawn contract; `thread-store/src/local/{rollout_lineage.rs,thread_history.rs,thread_history/**,thread_history_materialization.rs}` | Keeps official Runtime contracts usable by the Platform Host: relative paths written through `config/batchWrite` are validated against the owning Profile directory; a spawned child inherits the parent Thread's selected capability roots; app-server emits `thread/started` with parent and Role metadata before forwarding buffered child events; and paginated history reads safely catch a lagging or inconsistent SQLite projection up from canonical JSONL, including flattened token-count records. New enterprise Threads select V2 through the existing request-scoped config contract and do not add a V1 backend compatibility seam. | 7 | Focused relative Agent-file, selected-capability-root inheritance, multi-agent v2 child identity-ordering and Thread-store materialization tests; Manifest tests; adapter governed-versus-standard V2 configuration test; real Platform Supervisor case including restart/history recovery | Upstream validates relative config paths with their config-layer base, preserves selected capability roots across non-fork Agent spawns, publishes equivalent spawned-Thread identity before child lifecycle events, and makes official paginated history self-heal from a valid canonical rollout. |
...
| `retain-core`: Platform Runtime contract fixes | `app-server-protocol/src/{capability_manifest.rs,protocol/v2/thread.rs}`, `app-server/src/{config_manager_service.rs,request_processors/thread_processor.rs}`, `core/src/{config/mod.rs,agent/{control.rs,control/spawn.rs}}`, the narrow `thread_manager.rs` spawn contract, `thread-store/src/local/{rollout_lineage.rs,thread_history.rs,thread_history/**,thread_history_materialization.rs}`, and focused tests | Required so Profile-managed relative Agent Role files survive official config validation, non-fork child Threads keep their parent's selected Skills/Plugins/MCP capabilities, child events have an authoritative parent/Role identity before clients receive their lifecycle, and browser-facing paginated history converges back to valid canonical rollout JSONL after projection lag or a restart. New governed enterprise Threads explicitly enable V2 through the existing request config contract, so no product-specific backend selector is retained in Codex. |
