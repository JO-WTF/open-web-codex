...
| --- | --- | --- | --- | --- | --- |
| `provider-chat-transport` | `codex-api/src/chat_translate.rs`, `chat_translate_tests.rs`, `endpoint/chat.rs`, `sse/chat.rs`; isolated Core transport in `core/src/client/chat.rs` with minimal dispatch in `core/src/client.rs` | Translates third-party Chat Completions requests, streams, mixed/interrupted Tool calls and assistant output into Codex semantics. Responses namespaces, including MCP plugin tools, are flattened to Chat functions with request-scoped reverse mapping; Responses-only tools without complete Chat semantics remain hidden. The checked-in stream translator currently marks text accompanying Tool calls as `commentary` and text-only completion as `final_answer`; this is a known non-standard heuristic scheduled for replacement by the phase-neutral, ordered contract in `docs/chat-responses-translation-spec.md`. | 1 | `just test -p codex-api`; focused Core Chat endpoint, interrupted, namespace/MCP, unsupported-tool-policy, phase-neutral Message, explicit Provider extension, ordered Item and tool-call translation tests; real third-party live/history smoke | Upstream provides equivalent supported third-party wire translation, including ordered stream lifecycle, namespace/MCP restoration and explicit rather than Tool-presence-inferred phase/reasoning behavior. |
...
| `platform-runtime-contract-fixes` | `app-server-protocol/src/{capability_manifest.rs,protocol/v2/thread.rs}`, `app-server/src/{config_manager_service.rs,request_processors/thread_processor.rs}`; `core/src/{config/mod.rs,agent/{control.rs,control/spawn.rs}}` and the narrow `thread_manager.rs` spawn contract; `thread-store/src/local/{rollout_lineage.rs,thread_history.rs,thread_history/**,thread_history_materialization.rs}` | Keeps official Runtime contracts usable by the Platform Host: relative paths written through `config/batchWrite` are validated against the owning Profile directory; a spawned child inherits the parent Thread's selected capability roots; app-server emits `thread/started` with parent and Role metadata before forwarding buffered child events; paginated history reads safely catch a lagging or inconsistent SQLite projection up from canonical JSONL, including flattened token-count records; and the experimental `agents.multi_agent_v1_backend_override@1.0.0` capability gates a typed, request-scoped `thread/start`/`thread/fork.multiAgentBackend: "v1"` override. The override forces V1 above model metadata and V2 fork lineage without mutating persistent Profile or Project configuration; only the Platform's governed Supervisor requests it. | 7 | Focused relative Agent-file, selected-capability-root inheritance, multi-agent v2 child identity-ordering and Thread-store materialization tests; protocol start/fork serialization and Manifest tests; Core precedence test; app-server start/fork persisted-version tests; adapter governed-versus-standard parameter test; real Platform Supervisor case including restart/history recovery | Upstream validates relative config paths with their config-layer base, preserves selected capability roots across non-fork Agent spawns, publishes equivalent spawned-Thread identity before child lifecycle events, makes official paginated history self-heal from a valid canonical rollout, and exposes a capability-gated, request-scoped V1 selector with the same precedence (or V1 compatibility is no longer required). |
...
| `retain-core`: Platform Runtime contract fixes | `app-server-protocol/src/{capability_manifest.rs,protocol/v2/thread.rs}`, `app-server/src/{config_manager_service.rs,request_processors/thread_processor.rs}`, `core/src/{config/mod.rs,agent/{control.rs,control/spawn.rs}}`, the narrow `thread_manager.rs` spawn contract, `thread-store/src/local/{rollout_lineage.rs,thread_history.rs,thread_history/**,thread_history_materialization.rs}`, and focused tests | Required so Profile-managed relative Agent Role files survive official config validation, governed `thread/start`/`thread/fork` can fail closed on a declared V1 backend-override capability and force V1 above model metadata or V2 fork lineage without persistent config mutation, non-fork child Threads keep their parent's selected Skills/Plugins/MCP capabilities, child events have an authoritative parent/Role identity before clients receive their lifecycle, and browser-facing paginated history converges back to valid canonical rollout JSONL after projection lag or a restart. These are narrow corrections to existing v2 contracts, not new browser APIs. |
