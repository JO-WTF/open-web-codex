...
| --- | --- | --- | --- | --- | --- |
| `provider-chat-transport` | `codex-api/src/chat_translate.rs`, `chat_translate_history.rs`, `chat_translate_tests.rs`, `endpoint/chat.rs`, `sse/chat.rs`; current-Turn deferred-plan ownership in `core/src/{session/turn_context.rs,tools/router.rs,tools/handlers/tool_search.rs}` and focused Core tests; isolated Core transport in `core/src/client/chat.rs` with minimal dispatch in `core/src/client.rs` | Translates third-party Chat Completions requests, streams, mixed/interrupted Tool calls and assistant output into Codex semantics. Responses namespaces, including MCP plugin tools, are flattened to Chat functions with request-scoped reverse mapping. The retained bridge maps native client `tool_search` into one ordinary Chat function and replays its Tool result as history only. `TurnContext` owns the plan's lifetime while every per-sampling `ToolRouter` shares it: successful native search results are recorded by `ToolName` and schema, conflicting identities reject, and the Router emits the canonical next `Prompt.tools` for both Responses and Chat. Chat never scans ToolSearch history to construct callable tools. New Turn, cancellation, failure, and resume create or use a fresh plan, so previously loaded schemas cannot cross the Turn boundary. The Runtime still owns BM25 search, deferred registration, Tool identity, approval and dispatch. Plaintext native Agent messages retain the established assistant-side Chat projection, while collaboration `spawn_agent`, `send_message`, and `followup_task` calls carry the official direct-plaintext marker; encrypted Agent messages remain a typed preflight rejection. The checked-in stream translator currently marks text accompanying Tool calls as `commentary` and text-only completion as `final_answer`; this is a known non-standard heuristic scheduled for replacement by the phase-neutral, ordered contract in `docs/chat-responses-translation-spec.md`. | 1 | `just test -p codex-api`; focused Core current-Turn deferred-plan and collision tests; Core Responses prompt-loading integration; Chat HTTP/SSE `tool_search → mcp__map_utils.create_network_map_card → inline-map directive` E2E with exactly-one flattened tool regression; Chat request/stream/history ToolSearch round trip plus cross-Turn schema-isolation regression; real DeepSeek smoke remains a separate opt-in gate | Upstream provides equivalent third-party Chat ToolSearch lifecycle with one canonical current-Turn tool plan, or configured Chat providers expose native Responses `tool_search`. |
