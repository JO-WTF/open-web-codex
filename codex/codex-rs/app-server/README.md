...
- `modelProvider/models/list` — request a fresh model catalog from exactly the configured `providerId`; the Runtime resolves the provider's base URL and credentials from its config registry and never accepts browser-supplied URL or credential fields. A successful response contains bounded model summaries; a failed request returns a tagged `result` with one of `authentication`, `notFound`, `rateLimited`, `upstream`, `timeout`, `network`, `invalidJson`, `incompatibleSchema`, or `emptyCatalog`. This request does not switch the current provider, mutate Thread/Turn model state, or use the model-manager cache.
