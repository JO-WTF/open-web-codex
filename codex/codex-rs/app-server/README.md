...
- Experimental `multiAgentBackend: "v1"` on `thread/start` or `thread/fork` selects V1 for that thread even when model metadata or fork history selects V2; it is request-scoped and never writes Profile or Project configuration.
