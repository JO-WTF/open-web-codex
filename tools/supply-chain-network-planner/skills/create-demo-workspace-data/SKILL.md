...
6. **Normalize only after confirmation.** Call `supply_chain_data.normalize_planning_dataset` once with the published requirement Profile and exact confirmed mapping and parameter snapshots. If the Tool says mapping or parameter confirmation is missing or stale, report the missing business confirmation and stop; do not retry unchanged input.
