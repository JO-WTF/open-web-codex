...
from src.network.geo import (  # noqa: E402
    Coordinate,
    ProvinceBoundary,
    haversine_km,
    pearson_correlation,
...
NETWORK_BOUNDARY_URL = (
    "https://media.githubusercontent.com/media/wmgeolab/"
    "geoBoundariesArchive_3_0_0/7c8dbc599e312d9204e450aecfa66c204b8cf9b8/"
    "releaseData/otherReleases/geoBoundariesSSCU-3_0_0/IDN/ADM2/"
    "geoBoundariesSSCU-3_0_0-IDN-ADM2.geojson"
)
NETWORK_BOUNDARY_SHA256 = "2b08dc56f4b488f1a6c84093c6a302628feb0fa49efefa47cd9b1fee3bd129e3"
...
def generate_network_fixture(
    *, root: Path, replace: bool, download_missing_sources: bool
) -> dict[str, Any]:
    ...
    if root.exists() and replace:
        for generated_name in ("base", "current-coverage-extension"):
            shutil.rmtree(root / generated_name, ignore_errors=True)
    if root.exists() and any(
        item.name not in {"source", "base", "current-coverage-extension"}
    ...
    if not source_path.exists():
        if not download_missing_sources:
            raise FileNotFoundError(
                f"missing locked ADM2 source {source_path}; rerun with "
                "--download-missing-sources"
            )
        source_path.parent.mkdir(parents=True, exist_ok=True)
        _download(NETWORK_BOUNDARY_URL, source_path)
    source_digest = _sha256(source_path)
    if source_digest != NETWORK_BOUNDARY_SHA256:
        raise ValueError(
            "locked ADM2 source digest mismatch: "
            f"expected {NETWORK_BOUNDARY_SHA256}, observed {source_digest}"
        )
    for directory in (base, current_extension):
...
def main() -> None:
    ...
    if args.network_fixture:
        result = generate_network_fixture(
            root=args.network_output_dir.resolve(),
