...
首次标准化必须保留全部已确认候选仓；只有候选仓增删替换可用 candidate delta。需求、现网仓、当前分配、路线事实或报价变化必须完整归一化。需要地理补全时使用已确认国家的行政区目录，直到得到 `ready` `normalized_network_input.v1`；原样保留其 `{server, uri, resource_schema}`，不构造或枚举 URI。
