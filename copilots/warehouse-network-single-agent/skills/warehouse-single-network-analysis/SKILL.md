...
使用同一批精确 normalized input、route matrix 与可选 cost matrix 引用。用户只问当前仓网时，按目标时效和 `existing_only` 评估，并同时报告城市等权与需求量加权覆盖。没有 `current_assignments` 时只能称为 `optimized_existing_footprint`，不能称真实现状。
