...
def ordered_assignment(value: AssignmentResult) -> AssignmentResult:
    return value.model_copy(update={"rows": sorted(value.rows, key=lambda row: row.demand_city_id)})
...
def ordered_comparison(value: AssignmentComparison) -> AssignmentComparison:
    return value.model_copy(
        update={
            ...
            "coverage": sorted(value.coverage, key=lambda item: item.target_hours),
            ...
            "city_changes": sorted(value.city_changes, key=lambda item: item.demand_city_id),
        }
    )
...
def _assignment_rows(
    ...
    expected_total = sum((item.demand_quantity for item in demand.values()), start=Decimal(0))
    ...
    for city_id, row in rows.items():
        ...
        if warehouse is None:
            raise ValueError(f"delivery_{source}_assignment_warehouse_unknown:{row.warehouse_id}")
        if warehouse.warehouse_id not in active:
            raise ValueError(f"delivery_{source}_assignment_warehouse_inactive:{row.warehouse_id}")
