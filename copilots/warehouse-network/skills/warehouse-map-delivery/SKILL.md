...
只从精确分析、比较或分配结果生成 distribution、coverage 或 comparison GeoJSON。展示已有基线时携带精确 `baseline_ref`。`network_plan_comparison.v2` 的 `before_ref` 与 `after_ref` 可以是任意合法的 baseline、scenario 或 facility-location 结果；只要 Planner 已发布同一 prepared identity 的 comparison，就直接把该单一 `plan_comparison_ref` 传给 `prepare_network_comparison_map`，不分别拼装引用，也不为地图重算业务结果。仅查看一个结果的覆盖图时，仍把对应 `baseline_ref`、`scenario_ref` 或 facility result 作为 `assignment_result_ref` 传给 `prepare_network_coverage_map`。
...
低层样式调整时才按 `profile.discriminator_property`、`feature_types` 和实际非空字段构造筛选、标签与 hover；不要从业务名称或历史地图猜字段。单一结果可使用 `assigned_warehouse_id`、`distance_km`、`duration_hours`、`unit_cost`；comparison 使用 `before_*` 与 `after_*`，仓库状态使用 `before_active`、`after_active`、`added_facility`、`removed_facility`。缺失字段直接省略，不制作通用替代字段。
