...
const REPLY_CARD_CAPABILITIES: &[ReplyCardCapability] = &[ReplyCardCapability {
    ...
    input_schema: r#"{"title":"string","intent":"coordinates|location|route|distance|boundary|visualization","input_ref":"optional tool-result or artifact reference","artifact_id":"optional existing artifact reference","fallback_text":"string","points":"optional small point array","lines":"optional small line array","polygons":"optional small polygon array","geojson":"optional small FeatureCollection"}"#,
    artifact_policy: "Never inline large GeoJSON. Use input_ref or artifact_id for server-generated geospatial payloads. Small point/line/polygon previews are allowed.",
}];
...
For geospatial answers, including results from MCP maps tools or custom skills,
place only a compact marker in the final answer at the exact position where the
card should appear. Third-party model Providers can use this marker too; it is
plain text and does not require the OpenAI Responses API.
...
mod tests {
    ...
    #[test]
    fn generated_prompt_is_provider_neutral() {
        let prompt = build_reply_card_affordance_prompt(reply_card_capabilities());
        assert!(prompt.contains("Third-party model Providers can use this marker too"));
    ...
    fn prepares_normal_messages_with_generic_card_affordance() {
        let prepared = prepare_message_text_for_reply_cards("请查询两地路线");
        assert!(prepared.starts_with("请查询两地路线"));
        assert!(prepared.contains("The Web platform can render structured reply cards"));
    }
