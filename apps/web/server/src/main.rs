...
use open_web_codex_provider_service::secured::{
    AuthorizedProviderOperations, SecuredProviderService,
};
...
use open_web_codex_adapter::{real::RealCodexAdapter, CodexAdapter};
...
struct Cli {
    ...
    #[arg(long, env = "CODEX_HOME", required_unless_present = "migrate_only")]
    ...
    #[arg(
        long,
        env = "OPEN_WEB_CODEX_COPILOTS_ROOT",
    ...
    #[arg(
        long,
        env = "OPEN_WEB_CODEX_COPILOT_PREPARED_ROOT",
        required_unless_present = "migrate_only"
    )]
    ...
    #[arg(
        long,
        env = "OPEN_WEB_CODEX_COPILOT_BUILD_STORE_ROOT",
        required_unless_present = "migrate_only"
    )]
...
async fn main() -> anyhow::Result<()> {
    ...
    let master_key = MasterKey::from_base64(
        &std::env::var("OPEN_WEB_CODEX_MASTER_KEY")
            .map_err(|_| anyhow::anyhow!("OPEN_WEB_CODEX_MASTER_KEY is required"))?,
    )?;
    ...
    .await?;
    let copilots_root = cli
        .copilots_root
        .as_deref()
        .expect("Clap requires --copilots-root outside --migrate-only");
    let copilot_prepared_root = cli
        .copilot_prepared_root
        .as_deref()
        .expect("Clap requires --copilot-prepared-root outside --migrate-only");
    let copilot_build_store_root = cli
        .copilot_build_store_root
        .as_deref()
        .expect("Clap requires --copilot-build-store-root outside --migrate-only");
    let copilot_sources = copilot_installation::CopilotSourceRegistry::discover(
        copilots_root,
        copilot_prepared_root,
        copilot_build_store_root,
    )?;
    ...
    let codex_home = cli
        .codex_home
        .clone()
        .expect("Clap requires --codex-home outside --migrate-only");
    prepare_single_profile_auth_import(cli.import_codex_auth_from.as_deref(), &codex_home)?;
    let composition =
        copilot_installation::cold_start_composition(&copilot_store, &copilot_sources, &codex_home)
            .await?;
    ...
    let provider_service = SecuredProviderService::new(
        state.db.clone(),
        profile_binding.runtime_key.clone(),
        registry.clone(),
        configuration_secrets.as_ref().clone(),
    );
    let secret_environment = provider_service.startup_secret_environment().await?;
    let host_config = ProfileHostConfig::new(cli.profile_id.clone(), codex_home, &workspace_root)
    ...
    let providers: Arc<dyn AuthorizedProviderOperations> = Arc::new(provider_service);
    ...
    let native_visualizations = profile_binding.codex_home.clone().map(|codex_home| {
...
mod tests {
    ...
    fn command_line_accepts_trusted_copilot_directories() {
        ...
        assert_eq!(cli.codex_home, Some(PathBuf::from("/profiles/default")));
    ...
    #[test]
    fn command_line_requires_runtime_inputs_except_migration_only() {
        assert!(Cli::try_parse_from(["open-web-codex-server"]).is_err());
        assert!(Cli::try_parse_from(["open-web-codex-server", "--migrate-only"]).is_ok());
    }
