...
| Change Provider CRUD/model refresh | `src/services/webClient.ts`, `server/src/routes/providers.rs`, `crates/provider-service`, `crates/platform-contracts`; `src-tauri/src/shared/codex_core.rs` is compatibility-only |
