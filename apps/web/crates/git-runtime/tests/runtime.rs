...
async fn writes_uploaded_workspace_files_atomically_and_rejects_unsafe_paths() {
    ...
    assert!(std::fs::read_dir(checkout.root.join("data"))
        .unwrap()
        .all(|entry| !entry
            .unwrap()
            .file_name()
            ...
            .starts_with(".upload-")));
    assert!(runtime
        .write_file(workspace_id, "../outside.txt", b"blocked", false)
        .await
        .is_err());
    assert!(runtime
        .write_file(workspace_id, ".git/config", b"blocked", false)
        .await
        .is_err());

    assert!(matches!(
        runtime
            .write_file(workspace_id, "data/planning.csv", b"replacement", false)
            .await,
        Err(GitRuntimeError::FileAlreadyExists(path)) if path == "data/planning.csv"
    ));
    assert_eq!(
        std::fs::read(checkout.root.join("data/planning.csv")).unwrap(),
        b"city,demand\nJakarta,10\n"
    ...
    runtime
        .write_file(workspace_id, "data/planning.csv", b"replacement", true)
        .await
        .unwrap();
    assert_eq!(
        std::fs::read(checkout.root.join("data/planning.csv")).unwrap(),
