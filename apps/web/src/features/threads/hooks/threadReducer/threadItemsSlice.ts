  ...
  switch (action.type) {
    ...
    case "appendAgentDelta": {
      ...
      if (index >= 0 && list[index].kind === "message") {
        ...
        list.push({
          ...
          kind: "message",
          role: "assistant",
          text: stripLeadingProviderSentinel(action.delta),
        });
      }
    ...
    case "completeAgentMessage": {
      ...
      if (index >= 0 && list[index].kind === "message") {
        ...
        list[index] = {
          ...existing,
          text: stripLeadingProviderSentinel(action.text) || existing.text,
        };
      } else {
        list.push({
          id: action.itemId,
          kind: "message",
          role: "assistant",
          text: stripLeadingProviderSentinel(action.text),
        });
      }
      const updatedItems = prepareThreadItems(list, { maxItemsPerThread: state.maxItemsPerThread });
      const nextThreadsByWorkspace = maybeRenameThreadFromAgent({
