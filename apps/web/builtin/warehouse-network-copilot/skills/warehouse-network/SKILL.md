...
- Use the map provider to obtain navigation route facts. `create_map_card` is also available when the user explicitly requests an interactive map card; use only an exact returned map Resource reference and never use the card as intermediate data or a substitute for a final delivery. Final warehouse comparison maps still come from the warehouse-network delivery capability so they preserve the exact normalized, baseline, facility and comparison inputs and the Platform-readable final descriptor.
