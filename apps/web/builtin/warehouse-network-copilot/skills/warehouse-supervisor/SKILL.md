...
- 只展示需求城市和现有仓库分布时，传给 `network_agent` 的任务必须明确：复用当前 Thread 中精确的 `normalized_network_input.v1` ResourceRef，依次使用它可见的分布地图 Resource Tool 和地图卡片 Tool；默认不展示候选仓，不运行路线、成本、覆盖、模拟或优化，不调用导航，不创建或修改 Workspace 文件。
